...
**Замер — такой же код, и ошибка в нём выглядит как результат.** В этой главе таких ошибок было две. Замер отката — того самого хода назад — показал правильное число по неправильной причине: отметка «кто отвечал в прошлый раз» между вопросами не сбрасывалась, и выигрыш засчитывался механизму, который не срабатывал. Замер параллельного запуска двух специалистов показал «выигрыш 92%», в котором была загрузка модели, а не работа: последовательный прогон шёл первым и грел модель для второго.

Оба раза число выглядело правдоподобно, и заметить подвох удалось только по деталям, напечатанным рядом: имя одного специалиста там, где должно быть два, и подозрительно круглое преимущество. Отсюда правило: **печатайте в замерах не только итог, но и то, из чего он сложился.** Итог обмануть легко, а трейс — трудно.
