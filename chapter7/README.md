...
> **Про LangGraph.** Всё, что описано в этой главе, — узлы, рёбра, общее состояние, сохранение прогона — есть в готовом виде в LangGraph, и в рабочем проекте берут именно её. Здесь мы пишем своё, и ровно по одной причине: **эта глава про механизм, а не про интерфейс библиотеки.** Весь поток управления тут 187 строк, которые читаются целиком; с готовой библиотекой глава превратилась бы в рассказ о том, как вызвать её функцию добавления условного ребра.
>
> Разница видна на сохранении прогона: готовый механизм спрятал бы две вещи, которые ниже всплыли живыми ошибками, — на какой узел должен указывать снимок и почему узел с побочным действием обязан переживать повтор. Разобрав их руками один раз, дальше пользоваться готовым проще.
>
> Что́ наш граф не делает, а LangGraph делает: одновременное выполнение нескольких узлов на одном шаге, потоковую выдачу, правила слияния состояния и хранилища сложнее JSON-файла. Как только понадобится хоть что-то из этого — писать своё смысла нет.
...
|[Назад](../chapter6/README.md) | [📚 Оглавление](../README.md) | [Далее](../chapter8/README.md)|
