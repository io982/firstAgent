...
Отсюда пёстрая картина. Вот три модели, на которых писался курс:

| Модель | Нативный вызов | Проверено |
|---|---|---|
| `llama3.1:8b` | работает | да, [раздел 8.2](#82-замер-с-которого-началась-глава) |
| `qwen2.5-coder:7b` | не работает | да, там же |
| `qwen2_5coder3b_q5` | не работает | да, там же |

Обычно поддержку заявляют модели общего назначения, дообученные под
инструкции и вызов функций: линейка Llama 3.1 и новее, обычные (не Coder)
Qwen2.5-Instruct, Mistral, Command-R. Но «заявляют» и «работает» — разные
вещи, и проверяется это одним запросом; как именно — в конце этого раздела.
...
разбор, никаких регулярок. Логично было бы взять его и не мучиться.

Проблема в последней строке таблицы.
