...
from chapter2.agent import (
    SYSTEM_PROMPT as BASE_SYSTEM_PROMPT,   # промпт, который дополним
    execute_tool as base_execute_tool,     # диспетчер, который расширим
    extract_tool_calls,                    # парсер — без изменений
    is_safe_query,                         # проверка запроса — без изменений
    request_model,                         # ядро связи с Ollama (через Главу 1)
)
```

Дальше Глава 3 дописывает к промпту свои правила и оборачивает диспетчер
своим — тем, что знает про инструменты памяти, а всё остальное передаёт
дальше в `base_execute_tool`. Получается не копия и не форк, а слой поверх.

```mermaid
graph TD
    Base["chapter1.agent<br/>ядро: request_model, _safe_eval_node, запуск Ollama"]
    Base --> Ch2["chapter2<br/>реестр @tool, свой execute_tool,<br/>свой парсер, свой SYSTEM_PROMPT"]
