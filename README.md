...
| Глава | Тема | Код |
| --- | --- | --- |
| [Введение](#введение) | Выбор модели, установка, устройство репозитория | — |
...
Всё, что нужно знать и поставить до первой главы: какая модель подойдёт
и почему, как развернуть окружение, как устроен репозиторий и чего
опасаться до запуска Главы 4.
...
Первый вопрос, который возникает: какую модель брать? Ответ «побольше»
неверен, и это не мнение — в курсе есть замер, где модель на 3 миллиарда
параметров обходит модель на 7 миллиардов.

### Что делает модель пригодной для агента

Агенту нужно не то же, что чат-боту. Чат-бот должен интересно писать;
агент должен **делать ровно то, о чём договорились**, и не отклоняться.
Отсюда четыре свойства, в порядке важности:

**1. Следование формату.** Агент просит модель отвечать вызовом
инструмента в определённом виде. Модель, которая вместо этого пускается
в объяснения, ломает цикл. Это главное требование, и оно почти не связано
с «умом» модели.

**2. Обучение вызову функций.** Умение выдать вызов структурой, а не
текстом. Не врождённое: модель должна была видеть такие диалоги при
обучении. Подробно — в разделе [1.4](#14-два-способа-вызвать-инструмент).

**3. Размер контекста.** С Главы 6 агент кладёт в контекст найденные куски
кода, и окна на 4096 токенов перестаёт хватать. Все модели курса дают
32 768 — этого достаточно.

**4. Скорость.** На обучении важнее, чем кажется: разница между ответом
за 5 секунд и за 40 определяет, сколько экспериментов вы поставите
за вечер.

Размера в этом списке нет. Он влияет на качество формулировок, но
на способность вызвать инструмент — почти нет.

### Специализация важнее размера

Самое неочевидное для новичка. Модели одного семейства и размера могут
вести себя противоположно, если их дообучали для разного.

| | Coder-версии | Instruct-версии |
|---|---|---|
| Для чего дообучали | писать код | вести диалог, выполнять инструкции |
| Вызов функций | не давали при обучении | давали |
| Хороши в | генерации и объяснении кода | следовании формату, работе с инструментами |

Замер из Главы 8: `qwen2.5-coder:7b` не отдаёт нативные вызовы, а
`qwen2.5:3b` отдаёт — при том, что вторая **вдвое меньше**. Ту же картину
дал старый дефолт курса, Coder-3B: на вопрос «где реализован калькулятор»
он назвал строку 30 и сигнатуру `-> float`, которых в проекте нет, тогда
как Instruct-версия того же размера процитировала настоящий код.

Вывод: для агента берите **Instruct**, а не Coder. Даже если агент
занимается кодом.

### Разные роли — разные модели

Одна модель на всё — не обязательное условие. В курсе их три, и каждая
делает своё:

| Роль | Модель | Почему такая |
|---|---|---|
| Агент с инструментами | `qwen2.5:3b` (1.9 GB) | Instruct, нативные вызовы, влезает в 4 GB, 88 ток/с |
| Эмбеддинги для поиска | `nomic-embed-text` (274 MB) | Отдельный класс моделей: она не генерирует текст, а превращает его в вектор |
| Точные ответы по коду | `llama3.1:8b` (4.9 GB) | Не выдумывает номера строк, но требует 6 GB и втрое медленнее |
| Перефразировщик (Глава 2) | хватит `qwen2.5:1.5b` | Задача простая: переписать фразу. Держать ради неё большую модель незачем |

Мысль, которую стоит забрать: **качество поиска в Главах 6–7 от основной
модели не зависит вообще** — там работают эмбеддинги. Меняя основную
модель, вы меняете качество ответов, но не качество поиска. Это измерено
в разделе [7.7](#77-а-если-взять-модель-побольше).

### Чего не исправит никакая модель

Честно, чтобы не было завышенных ожиданий. Вопрос «где определён инструмент
агента калькулятор» провалили **все четыре** проверенные модели: вместо
поиска по проекту они вызывали калькулятор или отвечали не подумав.
Выбор инструмента при неоднозначной формулировке — задача архитектуры
агента, а не модели. Разбор — в разделе
[7.7](#77-а-если-взять-модель-побольше).

### Как проверить кандидата самому

Не по документации и не по флагу `tools` в `ollama show` — он врёт,
и почему, разобрано в [1.4](#14-два-способа-вызвать-инструмент). Проверка —
один запрос, десять строк кода; готовый скрипт есть там же, а в Главе 8
он превращается в функцию, которую агент выполняет при запуске:
...
Всё, что ниже, измерено на одной машине (GTX 1660 SUPER, 6 GB) в этом
курсе, а не взято из документации:

| Модель | В памяти | Размещение | Скорость | Нативные вызовы | Выдумывает детали |
|---|---|---|---|---|---|
| `qwen2.5:3b` | 2.4 GB | 100% GPU | 88 ток/с | да | почти нет |
| `qwen2_5coder3b_q5` | 2.4 GB | 100% GPU | 73 ток/с | нет | да, грубо |
| `qwen2.5-coder:7b` | 5.4 GB | 79% GPU | 24 ток/с | нет | нет |
| `llama3.1:8b` | 6.2 GB | 68% GPU | 16 ток/с | да | нет |

Строка про скорость выглядит противоречиво рядом с временем ответа: самая
медленная по токенам `llama3.1` отвечает быстрее, чем `qwen2.5-coder:7b`,
потому что говорит коротко и по делу.
...
### Своя сборка из GGUF (необязательно)

Модель `qwen2_5coder3b_q5` — не из библиотеки Ollama, командой `ollama pull` она не ставится. Это Q5-квантизация [Qwen2.5-Coder-3B](https://huggingface.co/Qwen/Qwen2.5-Coder-3B), собранная из GGUF-файла вручную. Если хотите ровно ту же конфигурацию, что в курсе, соберите её сами — это три действия.
...
> 💡 Такая сборка всё равно будет отвечать текстом, а не нативными `tool_calls` — но не из-за шаблона: с ним всё в порядке. Причина в самой модели, и разобрана она в разделе [1.4](#14-два-способа-вызвать-инструмент). Курс с самого начала парсит JSON из текста ответа, поэтому работает в обоих случаях.

### Шаг 5. Проверка: что модель на месте
...
> [1.4](#14-два-способа-вызвать-инструмент). Из трёх моделей,
> на которых писался курс, этот флаг объявляют все три, а работает он
> у одной.

### Шаг 6. Проверка: первый запрос к модели
...
Заодно вы увидели, из чего состоит вся работа с моделью: HTTP-запрос
со списком сообщений, ответ в JSON. Весь курс дальше — надстройка
над этими пятнадцатью строками.
...
- [Введение: выбор модели, установка, устройство репозитория](#введение)
  - [Выбор модели: какую брать и почему размер не решает](#-выбор-модели)
...
## 1.2 Что за файл мы разбираем

Окружение готово, первый запрос к модели вы уже сделали на шаге 6
установки. Дальше начинается сам агент — и прежде чем разбирать его
по частям, посмотрим на файл целиком, чтобы было понятно, куда мы движемся.

Полный код: → [chapter1/agent.py](chapter1/agent.py)

Семьсот строк в одном файле выглядят страшнее, чем есть: половину при
первом чтении можно пропустить. Та же карта продублирована в самом начале
файла, чтобы не переключаться на README:
...
**Весь агент — это пункты 2 и 6.** `SYSTEM_PROMPT` договаривается с моделью
о том, как она попросит вызвать инструмент, а `ask_agent` эту просьбу
исполняет. Всё остальное в файле обслуживает эту пару. Как именно они
работают, разбирает следующий раздел.

Про остальные пункты, чтобы не отвлекали:

**Инструменты (4)** — обычные функции, ничего особенного. Отдельного
взгляда заслуживает только калькулятор: он разбирает выражение в дерево
вместо `eval`, и почему — в разделе
[1.6](#16-безопасность-инструментов).

**Разбор ответа (5)** — самое неопрятное место главы. Три попытки
разобрать ответ подряд, костыль для путей Windows. Это выглядит избыточным
ровно до раздела [1.4](#14-два-способа-вызвать-инструмент), где станет
видно, на чём вообще держится формат ответа.

**Автозапуск Ollama (3) можно не читать вовсе.** Четыре функции, которые
поднимают сервер, если он не запущен, и прогревают модель. Полезно, когда
пользуешься агентом каждый день, и никак не связано с тем, как агент
устроен.

> 💡 Дальше по курсу структура повторяется: `agent.py` — точка входа
> и правила для модели, `src/` — то, что глава добавляет нового. Начиная
> с Главы 4 ядро больше не переписывается: главы подменяют в нём отдельные
> части, а как именно — в разделе
> [«Как устроен репозиторий»](#-как-устроен-репозиторий).

## 1.3 Архитектура ReAct-агента
...
(раздел [1.7](#17-когда-модель-выдумывает-инструмент)).

## 1.4 Два способа вызвать инструмент

### Некоторые модели умеют вызывать инструменты по-настоящему

В прошлом разделе агент заработал, и кажется, что вопрос закрыт. Но способ,
которым он вызывает инструменты, — не единственный и не главный.

Существует **нативный вызов**: модель обучена специально тому, что у неё
есть функции, которые можно попросить выполнить. Ей передают список функций
не текстом в промпте, а отдельным полем запроса, и просьбу она возвращает
не текстом, а отдельным полем ответа. Разбирать нечего — всё уже разобрано.

Умение это не врождённое, а **выученное**. Модель не «понимает», что такое
инструмент: её при обучении показывали тысячи диалогов, где после вопроса
пользователя шёл вызов функции в строго определённом виде, и она научилась
такие вызовы воспроизводить. Кого не учили — тот не умеет, как бы правильно
его ни просили.

Отсюда пёстрая картина. Вот модели, на которых писался курс:

| Модель | Нативный вызов | Проверено |
|---|---|---|
| **`qwen2.5:3b`** — наша по умолчанию | работает | да, [раздел 8.2](#82-замер-с-которого-началась-глава) |
| `llama3.1:8b` | работает | да, там же |
| `qwen2.5-coder:7b` | не работает | да, там же |
| `qwen2_5coder3b_q5` | не работает | да, там же |

Закономерность видна сразу: работает у моделей общего назначения,
дообученных под инструкции, и не работает у специализированных под код.
Обе Coder-версии в таблице — про написание кода, обе Instruct — про диалог,
и вызов функции это именно диалоговое умение.

Кроме перечисленных, поддержку обычно заявляют Mistral и Command-R.
Но «заявляют» и «работает» — разные вещи, и проверяется это одним запросом;
как именно — в конце этого раздела.

### Почему умеют не все

Четыре причины, и ни одна не про «плохую модель».

**Специализация.** Обе наши Qwen2.5-**Coder** — модели для написания кода.
Их дообучали на репозиториях, а не на диалогах с вызовами функций. Дело
не в силе модели, а в том, чему её учили: у той же линейки Qwen2.5 есть
обычные Instruct-версии, и с вызовами у них дела обстоят лучше, чем
у Coder-версий того же размера.

**Размер.** Вызов функции требует дисциплины: выдать строго определённую
разметку и ничего лишнего. Модели до 3B срываются на объяснения —
наша 3B в ответ на запрос с инструментами написала рассуждение и приложила
XML-блок, который никто не просил.

**Формат разный у всех.** Единого стандарта нет: Qwen ждёт теги
`<tool_call>`, Llama — свои служебные токены. Сервер должен узнать
именно тот формат, которому обучали конкретную модель.

**Сборка может всё сломать.** Модель — это файл весов плюс шаблон промпта.
Пересоберёте модель, переопределив шаблон, — поддержка вызовов исчезнет,
хотя веса остались прежними. Как раз поэтому в разделе
[«Своя сборка из GGUF»](#своя-сборка-из-gguf-необязательно) сказано
не трогать `TEMPLATE` без нужды.

Что из этого следует практически: **нативный вызов — приятная возможность,
на которую нельзя рассчитывать**. Курс поэтому построен на втором способе,
работающем везде. Дальше — чем они отличаются и почему выбор именно такой.

### То, что мы построили в 1.3, — самодельный протокол

Перечитайте, как в предыдущем разделе агент вызывал калькулятор. Модель
написала строку:
...
Откуда она узнала, что писать надо именно так? Из системного промпта —
мы сами её попросили:
...
Это **договорённость, которую мы выдумали**. Не стандарт, не требование
Ollama — просто текст в промпте. Отсюда её свойства:

| | Как есть в нашем агенте |
|---|---|
| Кто описал инструменты модели | Мы, обычным текстом в промпте |
| Кто задал формат ответа | Мы, там же |
| Кто разбирает ответ | Мы, функцией `extract_tool_calls` |
| Кто гарантирует формат | Никто |

Последняя строка — самая неприятная. Модель **не обязана** нас слушаться.
Она может ответить «Сейчас посчитаю: {"name": ...}», обернуть JSON
в тройные кавычки, перевести `name` на русский или выдать два вызова
подряд. Для неё это просто текст, который надо продолжить похоже
на примеры.

Именно поэтому `extract_tool_calls` в Главе 1 такая громоздкая: она
достаёт JSON из блоков ```` ```json ````, пробует разобрать любую фигурную
скобку в тексте и даже чинит неэкранированные слэши в путях Windows. Всё
это — заплатки поверх того, что формат ответа никем не гарантирован.

### Второй способ: сказать про инструменты серверу

У Ollama есть отдельное поле в запросе — `tools`. В него кладут описание
функций **машиночитаемо**, а не текстом в промпте:
...
Это то же самое описание, что мы писали словами в промпте, только
в виде структуры: имя, описание, какие параметры, какого типа, какие
обязательны. Формат называется JSON Schema.

Дальше происходит главное. Ollama сама разворачивает этот список в промпт
так, как ожидает конкретная модель, а её ответ разбирает обратно
и кладёт результат в отдельное поле:

```python
message = response["message"]

if message.get("tool_calls"):              # ← уже разобрано за нас
    for call in message["tool_calls"]:
        name = call["function"]["name"]
        args = call["function"]["arguments"]   # ← уже словарь, не строка
```

Такой вызов называется **нативным**: его поддерживает сам сервер,
а не наш промпт. `content` при этом обычно пустой — модель ничего
не «сказала», она вызвала функцию.

| | Самодельный (текстовый) | Нативный (`tools`) |
|---|---|---|
| Кто описал инструменты | мы, текстом в промпте | мы, структурой в поле `tools` |
| Кто разворачивает описание в промпт | мы | Ollama, по шаблону модели |
| Где приходит вызов | в `content`, текстом | в `tool_calls`, структурой |
| Кто разбирает ответ | мы, регулярками и JSON-парсером | Ollama |
| Аргументы приходят как | строка, которую надо разобрать | готовый словарь |
| Работает на любой модели | да | нет |

### Почему их приходится противопоставлять

Оба способа делают одно и то же — дают модели попросить о вызове функции.
Но выбрать нужно **один**: они не складываются.

Если описать инструменты в поле `tools` и заодно оставить в промпте
инструкцию «ответь JSON-вызовом», модель получит два указания сразу
и начнёт выполнять оба: вызовет инструмент нативно и тут же продублирует
вызов текстом. Не гипотеза — ровно на это мы наступили в Главе 8, там же
и разбор ([раздел 8.6](#86-ошибка-которая-нашлась-только-в-подробном-режиме)).

Нативный путь по таблице выглядит строго лучше: меньше кода, надёжнее
разбор, никаких регулярок. **Так и есть — в работе он лучше**, и модель
по умолчанию в этом курсе его поддерживает.

Тогда почему Глава 1 строит текстовый протокол руками? Ответ — в конце
раздела, а сначала стоит понять, почему нативный путь нельзя просто
включить и забыть.

### Почему флаг `tools` — не гарантия

Выше сказано, что нативный вызов есть не у всех, и объяснено это
со стороны модели: чему её учили. Но есть и вторая сторона — техническая,
и именно она объясняет самое неприятное: почему **проверить по документации
нельзя**.

Логично было бы посмотреть в `ollama show`, увидеть там `tools`
и успокоиться. Так делать нельзя. Чтобы нативный вызов сработал, должны
совпасть **три вещи**, и каждая живёт отдельно от других.
...
|---|---|---|---|
| `qwen2.5:3b` | да | вызов в ожидаемом формате | **заполнено** |
...
Все четыре объявляют `tools`. У двух ломается звено 2: модели семейства
...
Обратите внимание на первые две строки против последних двух: размер тут
ни при чём. Наша 3B справляется, а 7B-Coder — нет, хотя вдвое больше.
...
### Так почему же Глава 1 разбирает текст руками

Модель по умолчанию поддерживает нативные вызовы. Логично было бы взять
готовое поле `tools` и не писать ни одной регулярки. Но Глава 1 всё-таки
собирает протокол вручную, и причина не в совместимости.

**Текстовый протокол — не альтернатива нативному. Это то, из чего нативный
сделан.** Перечитайте три звена выше: шаблон разворачивает список
инструментов в текст промпта, модель выдаёт текст в условленном формате,
сервер этот текст разбирает. Ровно то же самое мы делаем руками — только
все три роли исполняем сами.

Отсюда практическая польза, а не академическая:

- когда у `llama3.1` приходит пустой `content`, вы знаете почему;
- когда Coder-модель игнорирует `tools`, вы знаете, какое звено сломалось;
- когда флаг в `ollama show` врёт, вы знаете, что именно он проверяет.

Читатель, начавший сразу с готового поля, при первой поломке останется без
инструментов диагностики: у него `tool_calls` пустой, и всё.

Поэтому раскладка в курсе такая:

| Главы | Протокол | Зачем |
|---|---|---|
| 1 | текстовый, собранный руками | увидеть, из чего состоит вызов инструмента |
| 2–7 | тот же | их темы — память, плагины, RAG — от формата вызова не зависят |
| 8 | нативный, с определением формата | перейти на промышленный способ |

> ⚠️ **Не переносите парсер Главы 1 в рабочий проект.** Вылавливать
> JSON регулярками из текста — учебный приём, чтобы показать механику.
> В настоящем коде используйте `tools` и `tool_calls`, а текстовый разбор
> держите запасным путём для моделей, которые нативный формат не отдают.
> Как это устроено — Глава 8.

## 1.5 Запуск и тестирование
...
## 1.7 Когда модель выдумывает инструмент
...
Подробный разбор — в разделе [«Выбор модели»](#-выбор-модели), здесь
короткая выжимка.
