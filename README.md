...
ollama pull nomic-embed-text      # эмбеддинги для Глав 5-6, ~274 MB
```
...
(GTX 1660 SUPER, 6 ГБ) с официальной `qwen2.5-coder:7b`. Индекс не меняли —
эмбеддинги считает `nomic-embed-text`, поиск от основной модели не зависит.

**Выдумывание 7B чинит.** Тот же вопрос, на котором 3B сочинила номер строки
и сигнатуру:

```text
Вы > где реализован калькулятор?

🤖 3B  > 30: def calculator(expression: str) -> float:     ← выдумано всё

🤖 7B  > В файле `chapter4/src/tools.py`:
...
7B процитировала код дословно и верно. Разница принципиальная: **3B
пересказывает по памяти, 7B копирует из Observation.** Заодно она перестала
называть номера строк и вместо этого приводит сам код — то есть то,
что можно проверить глазами.

**Две другие беды остаются.**

Модель по-прежнему путает упоминание с реализацией. В том же ответе 7B
уверенно продолжила: «калькулятор реализован также в `ci_smoke.py`
и `chapter6/test_search.py`». В первом — вызовы калькулятора в проверках,
во втором — список поисковых запросов. Реализации там нет.

И выбор инструмента не стал надёжнее, просто ошибается иначе:

```text
Вы > где определен инструмент агента калькулятор?

🤖 3B  > [вызывает calculator: 2+2] Калькулятор вернул 4.
🤖 7B  > Инструмент агента калькулятор не определен в предоставленных
         инструментах.                              ← поиск не вызывался вообще
```

3B полезла считать, 7B ответила не подумав. Ни одна не сходила в `ask_project`,
хотя нужный файл в индексе есть и по метрике находится в топ-3.

**Чего это стоит.** На 6 ГБ видеопамяти 7B помещается не целиком:

| | 3B (Q5) | 7B (Q4) |
|---|---|---|
| Размер в памяти | 2.4 ГБ | 5.4 ГБ |
| Размещение | 100% GPU | 79% GPU / 21% CPU |
| Скорость генерации | 73 ток/с | 24 ток/с |
| Ответ с вызовом инструмента | 10–20 с | 30–45 с |

**Вывод.** Если ответы агента должны быть точными — берите 7B, разница
в достоверности стоит трёхкратного замедления. Но чуда не ждите: 7B
перестаёт выдумывать, однако не начинает рассуждать. Отличать реализацию
от упоминания и надёжно выбирать инструмент — задачи архитектуры,
а не размера модели, и решаются они в следующих главах.

Переключение — одной переменной окружения, код править не нужно:

```powershell
$env:AGENT_MODEL = "qwen2.5-coder:7b"
```
...
| Глава | О чём |
|---|---|
| 8 | Нативные `tool_calls` вместо парсинга JSON из текста |
...
| Ответы по коду, где важна точность | qwen2.5-coder:7b | ~4.7 GB | Цитирует вместо выдумывания, втрое медленнее |
| Критик | qwen2.5:3b | ~2 GB | |

> 💡 На видеопамяти 6 ГБ 7B помещается не целиком (79% GPU / 21% CPU) —
> работает, но со скоростью около 24 токенов/с против 73 у 3B. Подробное
> сравнение с примерами ответов — в разделе [7.7](#77-а-если-взять-модель-побольше).
...
> **Готовы главы:** 1–7. План дальнейших — [ROADMAP.md](ROADMAP.md)
