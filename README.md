...
> [1.6](#16-два-способа-вызвать-инструмент). Из трёх моделей,
...
> 💡 Такая сборка всё равно будет отвечать текстом, а не нативными `tool_calls` — но не из-за шаблона: с ним всё в порядке. Причина в самой модели, и разобрана она в разделе [1.6](#16-два-способа-вызвать-инструмент). Курс с самого начала парсит JSON из текста ответа, поэтому работает в обоих случаях.
...
## 1.4 Что за файл мы разбираем

Первый запрос к модели заработал — дальше начинается агент. Прежде чем
разбирать его по частям, посмотрим на файл целиком, чтобы было понятно,
куда мы движемся.
...
[1.8](#18-безопасность-инструментов).
...
ровно до раздела [1.6](#16-два-способа-вызвать-инструмент), где станет
...
## 1.6 Два способа вызвать инструмент

### То, что мы построили в 1.5, — самодельный протокол
...
{"name": "search_in_directory", "arguments": {"query": "TODO"}}
    ↓
Ядро агента парсит JSON (Глава 1.6) и вызывает execute_tool()
    ↓
...
В разделе 1.6 курс сознательно выбрал текстовый парсинг: модель возвращает
...
| **ReAct** | Схема работы агента: рассуждение → действие → наблюдение → снова рассуждение, пока задача не решена (раздел 1.5) |
...
| **Шаблон промпта (template)** | Правило, по которому список сообщений превращается в один текст для модели: где теги ролей, как показать список инструментов. Хранится внутри модели, смотрится через `ollama show --template` (раздел 1.6) |
| **Function calling** | Способность модели попросить о вызове функции. Не свойство модели как таковой, а договорённость между моделью, шаблоном и сервером — ломается в любом из трёх мест (разделы 1.6 и 8.2) |
...
| Модель не вызывает tools | Текстовый формат вместо нативного | Парсить JSON из текста (Глава 1.6) |
