...
> 💡 **Про модель.** Курс писался на `qwen2_5coder3b_q5:latest` — Q5_K_M-сборке [Qwen2.5-Coder-3B](https://huggingface.co/bartowski/Qwen2.5-Coder-3B-GGUF) из GGUF-файла, поэтому именно её имя зашито в код как значение по умолчанию. Через `ollama pull` она не ставится — как собрать её самому, описано в разделе [«Своя сборка из GGUF»](#своя-сборка-из-gguf-необязательно).
>
