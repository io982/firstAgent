...
> [1.5](#15-два-способа-вызвать-инструмент). Из трёх моделей,
...
> 💡 Такая сборка всё равно будет отвечать текстом, а не нативными `tool_calls` — но не из-за шаблона: с ним всё в порядке. Причина в самой модели, и разобрана она в разделе [1.5](#15-два-способа-вызвать-инструмент). Курс с самого начала парсит JSON из текста ответа, поэтому работает в обоих случаях.
...
## 1.5 Два способа вызвать инструмент

### То, что мы придумали в 1.4, — самодельный протокол
