...
| [Глава 6](chapter6/README.md) | Поиск BM25: совпадение слов вместо близости, реранкер и ответ «в проекте этого нет» | [chapter6/agent.py](chapter6/agent.py) |
...
**Дальше по плану:** гибридный поиск,
мульти-агентные системы, coding-агент, безопасность, оценка качества. Состав и статус каждой главы — в [ROADMAP.md](ROADMAP.md).
...
- Модель эмбеддингов `bge-m3` (~1.2 GB) — нужна с Главы 4, где появляется поиск по документам; для глав 0–3 не нужна. Замеры Глав 4 и 5 сняты на `nomic-embed-text` (~274 MB), он тоже работает и занимает на гигабайт меньше видеопамяти — но находит русский вопрос заметно хуже (замер в Главе 6)
...
ollama pull bge-m3                # эмбеддинги, нужны с Главы 4, ~1.2 GB
...
# Главы 4-6 — нужна модель эмбеддингов: ollama pull bge-m3
...
graph TD
    ...
    Ch5 --> Ch6["chapter6<br/>+ поиск по словам BM25 и реранкер на LLM,<br/>ответ «в проекте этого нет» по проверяемому признаку,<br/>точное вхождение grep_code"]
```
