...
| [Глава 3](chapter3/README.md) | Context и Memory: обрезка, суммаризация, память на диске и core-блок | [chapter3/agent.py](chapter3/agent.py) |
...
- ✅ Держать три уровня памяти: историю диалога, факты на диске и core-блок,
  который агент правит сам и видит в каждом запросе
- ✅ Видеть, где промпт-защиты от инъекций работают, а где дают лишь иллюзию —
  с числами, а не с обещаниями

**Дальше по плану:** RAG и векторный поиск, поиск по кодовой базе,
гибридный поиск, мульти-агентные системы, coding-агент, безопасность,
оценка качества. Состав и статус каждой главы — в [ROADMAP.md](ROADMAP.md).
...
- Модель эмбеддингов `nomic-embed-text` (~274 MB) — понадобится в главах про RAG, которые ещё в работе; для глав 0–3 не нужна
...
ollama pull nomic-embed-text      # эмбеддинги, пригодятся позже, ~274 MB
...
**Что агент делает с вашим компьютером уже сейчас:**
...
| `update_core` | 3 | Переписать `chapter3/core_memory.json` — блок из трёх полей, который виден модели в каждом запросе. Все правки пишутся в `chapter3/core_memory.log` |
...
**Дальше будет опаснее.** По плану курса у агента появляются инструменты,
...
Насколько плохо это лечится промптом, измерено в [Главе 3](chapter3/README.md):
там та же атака приходит не с веб-страницы, а через пересказ собственного
диалога агента.

> ⚠️ **Важно:** как только такие инструменты у агента появятся — не давайте
> ему ходить на случайные ссылки и читайте команду в запросе на подтверждение
> прежде чем нажать `y`. Подтверждение, которое жмут не глядя, защитой быть
> перестаёт.
...
graph TD
    ...
    Ch2 --> Ch3["chapter3<br/>+ бюджет и обрезка контекста, сжатие истории,<br/>память на диске и core-блок, санитизация вывода инструментов"]
```
