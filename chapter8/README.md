...
Модель, специально обученная агентным задачам, строит планы без единой претензии — и всё равно не выигрывает у плана, собранного кодом, а идёт вдесятеро дольше. Вывод узкий, и его границы надо назвать: это верно **на задачах правки, где файл назван в самой задаче**. На задачах с нуля у кода ответа нет, и там планирует модель.
...
Модель, специально обученная агентным задачам, строит планы без единой претензии — и выигрыша над планом из кода не даёт, а идёт вдесятеро дольше. Отсюда умолчание `AGENT_PLANNER=fallback`.

И то, что тот же замер показал заодно, стоит сказать прямо: **работа вообще без плана даёт те же 5 из 5.** План в этой главе нужен не для качества правки, а чтобы человеку было что подтвердить до того, как файл изменён. Приём, который не улучшает результат, но делает работу проверяемой человеком, — нормальный приём; ненормально выдавать его за то, что он улучшает результат.

У замера есть и своя кривизна, и назвать её лучше самому: колонка «без претензий» считается тем же валидатором, который требует, чтобы последний шаг был проверкой, — а план из кода этим шагом всегда и заканчивается. В этой колонке код слегка подыгрывает себе; колонка «зелёные», по которой сделан вывод, от валидатора не зависит.
...
* **Смысл проверить нечем.** Все проверки главы механические: разбирается, импортируется, запускается, имена определены. Неверная формула производной проходит их все. Единственный приём, который на этот вопрос отвечает, — разбор моделью — на модели курса не ловит ничего.
