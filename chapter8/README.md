...
* **разбираться с зависимостями** — и с внешними (какого пакета нет в окружении), и с внутренними (какой `import` забыли написать); и то и другое считается из кода, а не спрашивается у модели;
...
Чего он **не** умеет, чтобы список не обещал лишнего: держать в голове правила и стиль вашего проекта — ни `CONTRIBUTING.md`, ни настройки линтера он не читает; работать в песочнице; проверять написанное не на Python — файл на JS он напишет и поправит, но всё вычислимое в нём устроено на разборе Python, и такой прогон кончается пометкой «нечем проверить»; и отвечать на вопрос «то ли это, о чём просили» — на него отвечает человек.
...
graph TD
    ...
    S -->|"есть ещё шаги"| S
    S --> D["deps<br/>пакеты и забытые импорты"]
...
Курс идёт на `qwen2.5:3b`, и до этой главы разрыв с моделями побольше был виден смутно: где-то ответ поверхностнее, где-то маршрут выбран не тот. Кодинг-агент показывает разрыв прямо, потому что у его работы нет полутонов: код либо запускается и делает, что просили, либо нет. Ниже — три слабости, на которые наткнулась эта глава, и рядом с каждой число из чужих замеров: технического отчёта Qwen2.5-Coder и блога Qwen2.5. Числа самой главы отдельного раздела не занимают — они стоят там, где о них речь, а печатают их замеры, живущие тестами с меткой `slow` (`python -m pytest chapter8/tests.py -m slow -s`): прогнать их можно самому.

**HumanEval** там — «по описанию напиши функцию», **IFEval** — «соблюди форму ответа: одним словом, без пояснений, в теги», **Aider** — правки в настоящем репозитории, где надо не только придумать код, но и попасть форматом.
...
Названный файл, впрочем, не всегда цель. «Сделай bat файл для запуска `calc.py`» называет `calc.py`, а просит батник. «Напиши тест на `calc.py`» просит `test_calc.py` — а план получался «создать `calc.py`», то есть агент переписывал файл, о котором просили один лишь тест. Заметить такое трудно: перезапись названного файла — законная операция, и ни одна проверка записи против неё не возражает. Отличается цель от ссылки порядком слов: «напиши тест на `calc.py`» — сначала тест, потом файл; «напиши `calc.py` и тесты» — наоборот, и там просят обе вещи.
...
Узел `deps` разбирает импорты `ast` — модулем стандартной библиотеки, который читает текст программы как дерево, а не как строки, — отбрасывает стандартную библиотеку и то, что уже установлено, и остаток предлагает поставить. Установка проходит через подтверждение человеком: `pip install` — это команда, скачивающая код из сети. Свои модули из этого списка вычёркиваются, даже если файла ещё нет на диске: живой прогон, где шаг `create calculator.py` провалился, а в тесте остался `import calculator`, кончился тем, что агент предложил поставить `calculator` с PyPI — и человек согласился.

Но зависимость бывает и **внутренней**, и на 3B она случается чаще внешней: пакет на месте, а строки `import` в файле нет. Код зовёт `math.sqrt(d)`, и программа падает `NameError` на первой же строке. Раньше это чинила модель, и чинила плохо: получив traceback «name 'math' is not defined», она ответила `import math` — и строка приехала в конец файла, после вызова, который падал.

Ответ здесь вычислим целиком, поэтому его считает код. Линтер называет имя, которого нет, а `env.forgotten_imports` оставляет те имена, что проходят два условия сразу:

* **обращается как модуль** — `math.sqrt`, а не просто `math`. Без этого условия импортом «чинилась» бы забытая переменная: на `print(nothing)` линтер говорит ровно то же самое, что и на `math.sqrt`;
* **есть в стандартной библиотеке** — весь `sys.stdlib_module_names`, а не список избранных модулей: на Python 3.13 это 289 имён, и спрашиваются они у самого интерпретатора. Стороннее имя не берётся намеренно: `requests.get(...)` без импорта — тоже забытый импорт, но лечить его установкой пакета с подходящим именем значит вернуть ту самую беду с `calculator`.

Не проходит ни одно условие — имя остаётся линтеру, и о нём скажут человеку. Так же поступает агент и с `sqrt(4)` без импорта: тут забыт `from math import sqrt`, но какой именно модуль имелся в виду, вычислить нельзя, а угадывать — не работа этого узла.

Дописывается импорт обычным путём записи: с подтверждением, показом diff и записью в журнал отката. Одним блоком на файл — три импорта не должны превращаться в три вопроса подряд, — и наверх, потому что место для импорта в Python ровно одно. Считается это не однажды: после того как файлы написаны, и заново после каждой удачной починки. Модель, переписывая функцию, о шапке файла не помнит, а круг, потраченный на забытый импорт, потрачен впустую.

### Проверка: только то, что не зависит от задачи

Этот раздел главы переписан после живых прогонов, и переписан сильно.
...
Слабой стороной оказалось то, что **подставить ввод нельзя, не зная области определения задачи.** Числа, подобранные под квадратное уравнение, объявили сломанным исправный калькулятор логарифмов; числа, годные любой школьной задаче, не смогли сказать ничего о программе, которой нужен файл на диске. Оба прогона разобраны в «Что не сработало», а вывод из них короткий: **любой фиксированный набор входных данных — это утверждение о задаче, которой мы не видели.** Не «числа подобраны плохо», а «числа подобрать нельзя».
...
Всё, что агент умеет посчитать про код, он считает разбором `ast`. А `ast` разбирает Python и больше ничего — и это не оговорка в одном месте, а свойство половины механизмов главы.

Проверить легко: попросим написать в пустом каталоге `app.js` с незакрытой скобкой в первой же строке.

```
app.js: файл создан (2 строк).
функций в карте: 0
проверено: нечем | нечем проверить: True
```

Заведомо сломанный файл записан без единого возражения: проверка синтаксиса перед записью на файл с другим расширением отвечает «да» не глядя. Карта функций пуста — она строится обходом `*.py`. Вердикта нет вовсе: линтер читает Python, проверять оказалось нечем, и человек получает работу с пометкой «не проверено», а не с обещанием, что она работает.

До места записи агент, впрочем, сначала не доходил вовсе. Имя для новой программы спрашивается у модели одним коротким вопросом, а её ответ сверялся со списком расширений, где были Python, батники и скрипты. Живой прогон, семь задач подряд — «напиши консольное приложение на JS / bash / C++ / C# / Pascal / Go / Java»: bash прошёл, остальные шесть дали план «создать `main.py`», и человек отклонил их один за другим. Язык модель понимала и отвечала `random_answer.js`, `main.cpp`, `Program.cs` — выбрасывала её ответ наша проверка.

Список делал две работы сразу, и в этом была ошибка. Первая работа — прочитать имя файла в предложении человека, и здесь закрытый список необходим: без него именем файла становится любое слово с точкой, включая «т.е.» и «версия 1.0». Вторая — проверить ответ модели на вопрос «как назвать файл», и вот здесь список языков не нужен: язык выбирает модель из контекста задачи, а коду остаётся убедиться, что ответ — одно имя файла, а не путь и не фраза. Теперь так и сделано, а закрытый список остался на своей работе и пополнился языками, которые агент способен написать.

Вторая беда была тише и потому хуже. Слово `app.js` именем файла не считалось, задача уходила по ветке «файл не назван», и адресом правки становился последний изменённый файл:

```
поправь функцию answer в app.js    было: правим run.sh    стало: правим app.js
```

Увидеть такое можно было только в diff — и это тот случай, ради которого diff показывают до записи, а не после.

Тем же списком расширений отвечали и на другой вопрос — «над чем мы работали в прошлый раз». Это адрес для реплики, которая файла не называет вовсе, и живой прогон поймал его уже после первой правки: агент написал `console_app.js`, следующая реплика «добавь, чтобы оно не закрывалось сразу» файла не назвала, адрес среди «того, над чем работали» не нашёлся — и агент завёл новый `app_stay_open.py` с `print("Привет, мир!")`, после чего объявил прогон зелёным: линтер по питоновскому файлу отработал и претензий не нашёл. Теперь оба списка считаются из одного места, вычитанием заметок и настроек из общего: два перечисления одного и того же разъезжаются, и в этой главе это случилось дважды.

Осталось одно исключение, и оно про слова, которые выглядят именами файлов, ими не будучи: `node.js`, `vue.js`. Отличить платформу от файла нечем, кроме одного признака — лежит ли такой файл в каталоге. Поэтому слово после предлога «на» принимается за имя только тогда, когда файл существует: «напиши приложение на node.js» — язык, «напиши тест на app.js» при существующем `app.js` — имя.

Один механизм границу всё же переходит, и устроен он иначе. Модель, которой велели вернуть одну функцию, приносит вместе с ней блок запуска — в изменённом виде, потому что правка была именно там. Выбросить его нельзя, это и есть то, о чём просили; а встаёт он на место функции, и старый блок остаётся ниже. Поэтому после каждой записи агент смотрит, не появился ли в файле второй блок с тем же заголовком: если одна из копий совпадает со старой дословно, лишняя — она.

| Что делает агент | на `.py` | на `.js` |
| :--- | :---: | :---: |
| прочитать имя файла в задаче | да | да |
| назвать файл, которого ещё нет | да | да |
| читать репозиторий, искать по файлам и символам | да | да |
| четыре формы правки | да | да |
| diff, подтверждение, снимок в git, откат | да | да |
| изменённый блок ложится поверх старого | да | да |
| карта функций и правка функции по её границам | да | нет |
| проверка синтаксиса перед записью | да | нет |
| пропавшие и лишние определения | да | нет |
| забытые импорты и повтор импорта | да | нет |
| вердикт проверки | линтер | «нечем проверить» |

Верхняя половина таблицы работает на любом языке, потому что не заглядывает внутрь синтаксиса: поиск Главы 5 индексирует и JS, и TypeScript, четыре формы правки текстовые, git не спрашивает, на чём написан файл. Нижняя половина — это `ast` целиком.

А строка про блок, который ложится поверх старого, стоит в верхней половине из-за своего адреса. Блок опознаётся по нулевому отступу, и заголовок его — просто строка: `def answer():`, `function answer() {` и `func answer() int {` попадают под одно правило, без списка ключевых слов на каждый язык. Первая версия этого механизма искала `if __name__ == "__main__":` и работала бы ровно на одном языке: в JS точка входа пишется как `if (require.main === module)`, а чаще не пишется никак.

Отсюда правило, которое переносится за пределы главы: **механизм переходит на другой язык ровно настолько, насколько его адрес виден глазом.** Всё, что адресуется деревом разбора, остаётся при своём языке, и дотянуть его до второго — это не поправить условие, а взять ещё один парсер.
...
Чтобы подставить ввод, надо знать, какие значения задача считает допустимыми. Числа были подобраны под квадратное уравнение — `1, -3, 2` даёт корни 1 и 2, единица первой в каждом наборе, — и калькулятор логарифмов трижды подряд, на двух моделях, объявлялся сломанным:

```
Последняя ошибка: ZeroDivisionError: float division by zero
```

`math.log(x, 1)` — деление на `log(1)`, то есть на ноль: программа была написана верно, невалидный ввод подали ей мы. Числа заменили на «годные любой школьной задаче» — ни нуля, ни единицы, ни отрицательных; следующей сломалась задача «прочитай ячейку A1 из файла Excel», где подставлять было нечего вовсе, а программа падала `FileNotFoundError`, потому что файла нет и взять его негде.
...
| :--- | ---: | :--- |
| Правка файла | 611 | начало файла — 2000 символов, около 1000 токенов |
...
| Имя файла для задачи | 227 | текст задачи |

Самый тяжёлый запрос конвейера — правка: 611 токенов правил плюс около 1000 токенов файла, всего около 1611 при окне 8192. Файл при этом обрезается по границе строки: последняя показанная строка целая, потому что цитировать её будет модель, а половину строки процитировать нельзя. Втрое меньше, чем один только промпт агента Главы 6.
...
774 быстрых теста, живой модели не требуют (один пропускается на Windows без прав на символические ссылки). Отдельно — 10 проверок на живой модели (`-m integration`) и 10 замеров главы (`-m slow -s`); и те, и другие по умолчанию не выбираются.
...
* **дописывать забытый импорт сам** — линтер называет имя, которого в файле нигде нет, а код оставляет из таких имён те, к которым обращаются как к модулю (`math.sqrt`, а не просто `math`) и которые есть в стандартной библиотеке Python: все 289 её имён на версии 3.13, спрошенные у самого интерпретатора. Остальное достаётся человеку — `requests.get` без импорта тоже забытый импорт, но лечить его установкой пакета с похожим именем значит угадывать;
* **не плодить того, что в файле уже есть** — модель на 3B, которую попросили переписать одну функцию, приносит вместе с ней и импорт, и блок запуска программы. При записи копия, совпадающая со старой дословно, убирается, а изменённая ложится поверх старой, а не рядом с ней;
...
**Механизм переходит на другой язык ровно настолько, насколько его адрес виден глазом.** Почти всё, что агент этой главы умеет посчитать про код, он считает разбором `ast` — модулем стандартной библиотеки Python, который превращает текст программы в дерево. На файле, написанном не на Python, от этого не остаётся ничего: ни карты функций, ни проверки синтаксиса перед записью, ни вердикта линтера, и прогон кончается пометкой «нечем проверить». А то немногое, что границу переходит, устроено иначе: кусок кода опознаётся по нулевому отступу, и заголовок его — просто строка, поэтому `def answer():`, `function answer() {` и `func answer() int {` попадают под одно правило, без списка ключевых слов на каждый язык. Правило стоит помнить, выбирая, на чём строить свой инструмент: разбор даёт точность в одном языке, текст — грубость во всех.
