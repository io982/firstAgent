...
Три слова в заголовке — это три части главы, и порядок в них не случаен.

* **Код.** Двадцать новых инструментов (семь файловых, четыре запуска, пять git, четыре про окружение) и конвейер из девяти узлов, который ведёт задачу от плана до отчёта. Файл целиком приходит от модели обычным текстом, а не полем JSON, — и это самый крупный выигрыш главы;
* **Правка.** Уже написанное правится по месту, а не переписывается: агент выбирает функцию по карте проекта и меняет её одной из четырёх форм. Задача при этом может не называть ни файла, ни функции — «сделай, чтобы приложение выводило ещё и производную» тоже адрес;
* **Ревью.** Его делает человек. Агент показывает каждую правку diff-ом до записи и проверяет ровно то, что проверяется без знания задачи: разбирается ли файл и нет ли в нём имён из ниоткуда. Не согласен человек или красный линтер — всё сделанное откатывается по снимку в git.

Главная мысль: **обвязка не делает модель умнее — она режет многоходовую работу на одноходовые вопросы и делает неудачу обратимой.** Каждый приём этой главы устроен одинаково: там, где ответ вычислим, отвечает код; там, где нужна модель, ей задают вопрос с готовым списком ответов; а то, что она всё-таки сделала, показывается человеку и при отказе стирается.
---
...
| Проверка сразу после правки | Aider | узел `verify`, круг починки — но проверяется только универсальное | «Что не сработало» |
...
Отдельная ветка осталась одна: план обещал написать тесты, а их нет. Это тоже проверяется без знания задачи — файла либо нет, либо есть.

#### Что агент теряет и что возвращает

Тем, что перестаёт говорить «работает». Он говорит, что именно проверил:
...
Самый крупный приём, который глава выбросила, и выбросила уже после того, как он был написан, замерен и описан.
...
| 2 | Нужен ли планировщик-модель на правках | нет: у модели претензии к шести планам из десяти и вдвое дольше |
...
без плана (файл назван)          10/10         10/10           10/10          10/10      18.1
план из кода                      9/10         10/10           10/10          10/10      20.5
qwen2.5:3b                        8/10         10/10            4/10            5/10      47.9
Nanbeige4.2-3B-Base.Q4_K_M        8/10         10/10           10/10          10/10     311.9
```

Первое, что видно: **по зелёным прогонам все четыре способа неразличимы.** Десять, девять, восемь и восемь из десяти — это разброс, а не порядок. Планирование в этой главе результат правки не улучшает.
...
Настоящая разница между планировщиками не в зелёных, а в двух других колонках. **Претензии к самому плану** — выдуманный путь, отсутствующий шаг — у `qwen2.5:3b` в шести планах из десяти, у кода ни в одном. И **время**: 48 секунд против 20. Специализированная на агентных задачах Nanbeige4.2-3B планы строит чисто, как и код, но берёт впятнадцатеро дольше — 312 секунд. Отсюда умолчание `AGENT_PLANNER=fallback`: вторая модель не даёт ничего, чего не даёт код.

Последняя колонка — про сам замер, и появилась она после разбора его кривизны. Претензию «последний шаг не test» план из кода не получает **никогда**: он этим шагом и заканчивается по построению. То есть одна строка валидатора автоматически ставила коду плюс, а модели минус, ничего при этом не измеряя. «Без придирки» считает претензии без неё — и видно, что подыгрывание стоило модели ровно одного случая из десяти: 4/10 против 5/10. Вывод от этого не меняется, но узнать это можно было только измерив, а не рассудив.

*Числа в этой таблице сняты дважды: до того, как проверка свелась к линтеру, и после. Порядок строк не изменился, а времена упали втрое-вчетверо — прогон перестал гонять тесты и запускать программы на каждом круге. Судья при этом остался прежним по сути: замер сам прогоняет тесты задачи, а не спрашивает у агента, справился ли агент.*
...
* **Смысл проверить нечем — и теперь это сказано прямо.** Агент проверяет разбор и имена; ни то, ни другое не отвечает на вопрос «то ли это, о чём просили». Неверная формула производной проходит обе проверки, калькулятор логарифмов, считающий корень вместо логарифма, — тоже. Единственный приём, который на этот вопрос отвечает, — разбор моделью — на модели курса не ловит ничего. Судья здесь человек, и глава больше не делает вид, что это не так.
