# 🛠️ Глава 8. Кодинг-агент: пишет код, проверяет проверяемое и откатывает неудачу

> **Цель главы:** собрать агента, который принимает задачу на написание кода обычными словами, показывает план и ждёт подтверждение, пишет файлы, проверяет в них то, что проверяется без знания задачи, — и, если проверка не прошла, возвращает каталог в то состояние, в котором он был до начала.
...
* **проверяет то, что проверяется без знания задачи.** Файл разбирается, имён из ниоткуда нет — и это всё, что можно утверждать о чужом замысле со стороны. Запускать программу и подставлять ей ввод агент пробовал и перестал: чтобы подставить ввод, надо знать область определения задачи, а её знает только человек;
