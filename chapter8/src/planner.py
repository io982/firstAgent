...
Пять задач правки по два прогона: план от `qwen2.5:3b` — 8 зелёных
из 10 за 48 секунд, план из трёх шагов, собранный кодом, — 9 из 10
за 20. По зелёным это разница в один случай, то есть шум; настоящая
разница в соседней колонке отчёта — претензии к САМОМУ плану
(выдуманный путь, отсутствующий шаг) у модели в 6 планах из 10,
у кода ни в одном. И вдвое по времени.
...
# `qwen2.5:3b` дал 8 зелёных из 10 за 48 с, план из трёх шагов без
# всякой модели — 9 из 10 за 20 с. Один случай разницы по зелёным —
# это шум; настоящая разница в соседней колонке: претензии к самому
# плану (выдуманный путь, отсутствующий шаг) у модели в 6 планах
...
# слово с точкой, включая «т.е.» и «версия 1.0».
#
# Языки, которые агент не умеет проверять, в списке всё равно есть,
# и это не недосмотр. Живой прогон показал, чего стоило их отсутствие:
# просьба «поправь функцию answer в app.js» имени в себе не находила,
# уходила по ветке «файл не назван» и адресовалась последним изменённым
# файлом — агент открывал `run.sh`. Писать файл на другом языке агент
# умеет; значит, и прочитать его имя в задаче обязан.
CODE_SUFFIXES = (
    ".py", ".txt", ".md", ".json", ".toml", ".cfg", ".ini", ".yml", ".yaml",
    ".bat", ".cmd", ".sh", ".ps1",
    ".js", ".mjs", ".cjs", ".jsx", ".ts", ".tsx", ".go", ".java", ".cs",
    ".cpp", ".cc", ".c", ".h", ".hpp", ".rs", ".rb", ".php", ".kt",
    ".swift", ".sql", ".pas", ".lua",
)

# Предлоги, после которых слово с точкой — обычно название платформы,
# а не файла: «напиши на node.js», «сделай на vue.js». Слово после них
# принимается за имя, только если такой файл в каталоге ЕСТЬ.
PLATFORM_PREPOSITIONS = ("на", "используя", "через")

# Расширения, которые кодом не являются: заметки, настройки, данные.
NOT_CODE_SUFFIXES = (".txt", ".md", ".json", ".toml", ".cfg", ".ini", ".yml", ".yaml")
...
#
# А вот считаться список должен ИЗ CODE_SUFFIXES, а не писаться руками.
# Пока он был отдельным перечислением из пяти питоновских и скриптовых
# расширений, живой прогон дал вот что: агент написал `console_app.js`,
# следующая реплика «добавь, чтобы оно не закрывалось» файла не назвала,
# адрес искался среди «того, над чем работали» — и `.js` там не было.
# Агент завёл новый `app_stay_open.py` с `print("Привет, мир!")`
# и объявил его зелёным. Два списка расширений, которые правят руками
# по отдельности, разъезжаются — это уже второй раз в этой главе.
RUNNABLE_SUFFIXES = tuple(s for s in CODE_SUFFIXES if s not in NOT_CODE_SUFFIXES)
...
def named_file(task: str) -> str:
    ...
    Про существование файла здесь не спрашивают: этот вопрос задают
    выше, и ответы на него ведут в разные стороны. Исключение ровно
    одно — слово после предлога «на». `node.js` и `vue.js` выглядят
    именами файлов и ими не являются: «напиши приложение на node.js»
    называет платформу. Отличить одно от другого можно только тем,
    лежит ли такой файл в каталоге, — и в этом одном месте вопрос
    о существовании задаётся.
    """
    words = [word.strip(".:;!?()[]\"'") for word
             in task.replace(",", " ").replace("«", " ").replace("»", " ").split()]
    for place, candidate in enumerate(words):
        if not candidate.lower().endswith(CODE_SUFFIXES):
            continue
        try:
            path = guard.resolve_path(candidate)
        except guard.OutsideWorkspace:
            continue
        before = words[place - 1].lower() if place else ""
        if before in PLATFORM_PREPOSITIONS and not path.is_file():
            continue
        return candidate
    return ""
...
2. Латиницей, строчными буквами, слова через подчёркивание — если
   только язык не требует иного: в Java имя файла повторяет имя класса.
...
# Имя файла, каким его может вернуть модель: начинается с буквы или
# цифры, дальше буквы, цифры, точка, дефис, подчёркивание — и расширение.
FILENAME_SHAPE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.\-]*\.[A-Za-z0-9]{1,10}$")

# Расширения, которых у ответа модели быть не может. Список не про
# языки — про то, что агент пишет ТЕКСТ. Имя `calc.exe` в ответ
# на «напиши калькулятор» означает не выбор языка, а непонимание
# задачи, и заводить такой файл незачем.
BINARY_SUFFIXES = (
    ".exe", ".dll", ".so", ".dylib", ".bin", ".msi", ".jar", ".war",
    ".class", ".o", ".obj", ".lib", ".a", ".pyc", ".zip", ".tar", ".gz",
    ".7z", ".rar", ".pdf", ".png", ".jpg", ".jpeg", ".gif", ".ico",
    ".mp3", ".mp4", ".db", ".sqlite",
)


def looks_like_filename(name: str) -> bool:
    """Похоже ли на имя файла — по форме, а не по списку языков.

    Раньше на этом месте стоял `CODE_SUFFIXES`, и это была ошибка.
    Список знал Python и скрипты, а модель на задачи про JS, C++, C#,
    Go и Java отвечала `random_answer.js`, `main.cpp`, `Program.cs` —
    то есть определяла язык из контекста задачи верно. Её ответ
    выбрасывался, подставлялся `main.py`, и живой прогон дал пять
    планов подряд «создать main.py» для программ, которые на Python
    писать никто не просил.

    Язык выбирает модель. Коду остаётся то, чего модель не
    гарантирует: что ответ — одно имя, а не путь, не фраза и не
    пустая строка.
    """
    if not name or " " in name or name.lower().endswith(BINARY_SUFFIXES):
        return False
    return bool(FILENAME_SHAPE.match(name))
