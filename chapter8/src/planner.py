...
def remembered_file() -> str:
    ...
    Проверяется, что он есть В ТЕКУЩЕМ рабочем каталоге. Память живёт
    дольше каталога: человек сменил каталог или взялся за другой проект,
    а запись осталась, и без проверки правка ушла бы по адресу из чужой
    сессии. Нет файла — памяти как будто нет.
