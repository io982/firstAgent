...
живёт в оперативной памяти и умирает вместе с процессом; `git diff`
переживает перезапуск и показывает разницу, даже если правки делал
не агент.

Вторая — **точка, к которой можно вернуться**. Откат из `guard`
восстанавливает файлы одной сессии. Коммит — это отметка, к которой
можно вернуться через неделю.
