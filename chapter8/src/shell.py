...
Где здесь проходит граница. Подтверждение спрашивается перед произвольной
командой и перед запуском файла, но НЕ перед `run_tests` и `run_lint`:
у этих двух форма команды фиксирована, а в конвейере они вызываются
на каждом круге, и вопрос человеку на каждом круге превратился бы
в кнопку, которую жмут не глядя.

У этого решения была дыра, и закрыта она отдельно. `pytest` выполняет
`conftest.py` проекта до первого теста, а это произвольный код — то
есть исключение, сделанное ради удобства, пропускало чужой код без
единого вопроса. Теперь вопрос задаётся, но только если `conftest.py`
в каталоге ЕСТЬ, и только один раз на каталог (`guard.allow_pytest`).
Нет файла — нет и вопроса, прогон идёт как раньше.
...
def run_tests(target: str = "") -> str:
    ...
    # Единственный вопрос, который этот инструмент задаёт человеку,
    # и задаёт он его только при живом conftest.py: pytest выполнит
    # его до первого теста, а это произвольный код. Нет файла — нет
    # и вопроса, см. guard.allow_pytest.
