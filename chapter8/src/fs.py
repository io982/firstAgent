...
# Каталоги, в которые агент не заходит, — список общий с `guard`:
# тем же списком он ищет conftest.py, и два разных ответа на вопрос
# «куда не заходим» рано или поздно разъехались бы.
SKIP_DIRS = guard.SKIP_DIRS
...
def _clip(text: str, limit: int = OUTPUT_LIMIT) -> str:
    ...
    if len(text) <= limit:
        return text
    head = text[:limit]
    # Режем по границе строки, а не по символу. Обрубок посреди строки
    # процитировать нельзя, а цитата «было/стало» — форма правки
    # по умолчанию: модель, увидевшая в конце `import VENV_DIR, clip,`,
    # якорем такое сделать не может. Целая строка короче, зато ею
    # можно пользоваться.
    #
    # Оговорка про «больше половины»: в минифицированном файле весь
    # текст лежит одной строкой, и граница строки нашлась бы сразу
    # за заголовком — модель получила бы заголовок и ни байта кода.
    # Там полезнее обрубок: процитировать его всё равно нельзя,
    # но видно хотя бы, что за файл.
    edge = head.rfind("\n")
    if edge > limit // 2:
        head = head[:edge]
    return head + f"\n\n[...обрезано: показано {len(head)} символов из {len(text)}]"
