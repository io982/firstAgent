...
# --------------------------------------------------------------------
# ВЫБОР МЕСТА: КАКУЮ ФУНКЦИЮ ПРАВИТЬ
# --------------------------------------------------------------------

# Ответ «ни одна не подходит». Он обязан быть в перечислении, и это
# не мелочь: `enum` не оставляет модели выбора вне списка, поэтому
# без такого пункта она ВЫНУЖДЕНА назвать какую-нибудь функцию даже
# тогда, когда правка ни к одной не относится. Грамматика, у которой
# нет ответа «не знаю», получает выдуманный ответ.
#
# Но полагаться на этот пункт нельзя, и это показал замер главы
# (TestPlaceChoice), четыре задачи с местом и две без, по два прогона:
#
#     модель                    нашёл нужную   сказал «ни одна»
#     qwen2.5:3b                        8/8                0/4
#     qwen2_5coder3b_q5                 6/8                0/4
#     qwen2.5-coder:3b                  6/8                0/4
#     qwen2.5-coder:7b                  8/8                2/4
#     llama3.1:8b                       2/8                0/4
#
# Две колонки — две разные способности, и смешивать их в одно число
# нельзя. ВЫБРАТЬ функцию из списка модель умеет: на модели курса
# 8 из 8, и это при том, что ни одна задача не называет функцию
# по имени — текстовый поиск не нашёл бы ни одной. ОТКАЗАТЬСЯ
# от выбора она не умеет почти нигде: вместо «ни одна» называет
# первую попавшуюся.
#
# Отсюда правило: РЕШЕНИЕ «правка или новый файл» принимает код
# (`plan_kind`), а у модели спрашивают только «какую из этих». Вопрос,
# на который у неё нет хорошего ответа, ей не задают.
NOTHING_FITS = "ни одна не подходит"

CHOOSE_RULES = """Тебе дан список функций проекта и задача. Кода не пишешь.

Твоя работа — назвать ОДНУ функцию, которую надо изменить ради этой задачи.

Правила:
1. Имя бери из списка как есть, целиком.
2. Если задача про то, что программа ПЕЧАТАЕТ или СПРАШИВАЕТ у человека,
   это почти всегда функция, где стоят input и print, а не вычисление.
3. Если менять надо не существующую функцию, а добавить новую, отвечай
   «{nothing}».
4. Если ни одна функция к задаче не относится, отвечай «{nothing}»."""


def choose_schema(wanted: list[str]) -> dict:
    """Схема ответа «какую функцию править»: одно имя из перечисления.

    Ради этой схемы карта и заводилась. Придумать имя модель не может —
    его нет в грамматике; перепутать файл не может — имя однозначно
    указывает и на файл, и на строки. Остаётся ровно один способ
    ошибиться: выбрать не ту функцию, и это видно человеку в плане
    до того, как что-то изменится.
    """
    return {
        "type": "object",
        "properties": {
            "name": {"type": "string", "enum": [*wanted, NOTHING_FITS]},
            "why": {"type": "string"},
        },
        "required": ["name"],
    }
...
def choose(task: str, path: str = "") -> Definition | None:
    """Функция, которую просят изменить. None — «такой нет, нужна новая».

    `path` сужает список до одного файла: если место уже известно
    до файла, спрашивать про весь проект незачем, а короткий список
    модель читает внимательнее.

    Перед выбором недостающие назначения добираются у модели — выбирать
    по одним именам всё равно что выбирать по оглавлению без заголовков.
    Это стоит одного запроса и кэшируется, так что платится один раз
    на функцию, а не на каждую правку.
    """
    items = [item for item in scan() if not path or item.path == path]
    items = [item for item in items if item.kind != "класс"][:MAP_LIMIT]
    if not items:
        return None
    describe(items)
    ...
    listing = "\n".join(item.line() for item in items)
    messages = [
        {"role": "system", "content": CHOOSE_RULES.format(nothing=NOTHING_FITS)},
        {"role": "user", "content": f"Функции проекта:\n{listing}\n\nЗадача: {task}"},
    ]
    try:
        raw = request_model(messages, response_format=choose_schema([item.name for item in items]))
        name = str(json.loads(raw).get("name", "")).strip()
    except Exception:  # noqa: BLE001
        return None

    if not name or name == NOTHING_FITS:
        return None
    # Сверка с картой всё равно нужна. Схема гарантирует, что имя
    # из списка, но список собран ДО запроса, а файл за это время мог
    # измениться — например, предыдущим шагом того же плана.
    for item in items:
        if item.name == name:
            return item
    return find(name)
