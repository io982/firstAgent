...
**LLM** — это движок для предсказания текста. У неё нет «рук» (она не может открыть файл или запустить приложение) и нет памяти между вызовами (каждый запрос — чистый лист).
...
>Иногда оркестратор называют частью харнеса, если он просто вызывает LLM и инструменты. 
>Но полноценный агент — это не только вызов, а ещё и автономный цикл: 
>получил задачу → подумал → выбрал инструмент → выполнил → посмотрел результат → решил, что дальше
...
> Парсинг текста хрупкий: лишний пробел или перевод модели на русский язык ломают логику. JSON либо валиден, либо нет. Современные модели отлично генерируют JSON, что делает оркестратор в разы надёжнее.
...
* **`request_model(messages: list, response_format: dict | None = None)`**: Отправляет HTTP POST-запрос на `http://localhost:11434/api/chat`. Ключевые детали: `temperature: 0.1` (для детерминированного JSON) и `num_ctx: NUM_CTX` (явный лимит контекста — без него Ollama возьмёт своё значение по умолчанию, и вы не будете знать, с каким окном работаете).

  > Параметр `response_format` в этой главе не используется и по умолчанию равен `None` — на поведение агента он не влияет. Схема ответа и ограниченная генерация появятся там, где станут нужны; здесь мы сначала смотрим, как ломается разбор свободного текста.
...
> живёт один вызов `ask_agent` и содержит от силы десяток сообщений.
...
* **Несуществующий инструмент**: Модель выдумала имя. `execute_tool` возвращает «инструмент не найден» и список доступных — этот текст уходит модели наблюдением, и на следующей итерации она выбирает из настоящего списка. Отдельной проверки до вызова нет намеренно: она означала бы второй разбор ответа и второй список инструментов рядом с первым.
...
2. **Валидация запросов**: Функция `is_safe_query()` проверяет запрос на подозрительные паттерны:
   ```python
   SUSPICIOUS_PATTERNS = [
       r"игнорируй.*(систем|system)",   # и «системный промпт», и «system prompt»
       r"забудь.*инструкц",
       r"ignore.*system",
       # ...
   ]
...
* ✅ проходить цикл ReAct: подумать, вызвать инструмент, посмотреть на результат и решить, что дальше;
* ✅ договариваться с моделью о формате вызова — через строгий JSON и few-shot примеры, которые для 3B работают лучше правил;
* ✅ не падать: сломанный JSON, выдуманное имя инструмента, неверные аргументы, деление на ноль — всё это возвращается модели наблюдением, а не исключением Python;
* ✅ останавливаться в двух случаях из двух: получен финальный ответ или исчерпан лимит итераций;
* ✅ отклонять прямые попытки инъекции по списку паттернов — до того, как запрос уйдёт модели;
* ✅ поднимать Ollama и прогревать модель, чтобы первый ответ не ждал загрузки весов.

Чего он **не** умеет, и это видно уже на двух инструментах:

* формат ответа держится на **просьбе** в промпте. Модель может обернуть JSON в markdown, дописать пояснение до и после, оборвать кавычку — и парсер занимается угадыванием;
* имена параметров модель берёт только из примеров: описание инструмента собирается из `__doc__`, но сигнатуру не показывает;
* новый инструмент добавляется в двух местах — функция в `TOOLS` и few-shot пример в тексте промпта. С двумя инструментами это незаметно, с восемью код и промпт разъедутся молча;
* история живёт внутри одного вызова `ask_agent` и умирает вместе с ним: спросить «а что я говорил до этого?» невозможно.

Первые три пункта — точка входа в следующую главу.
