...
# ====================================================================
# Единственный список на весь курс. Следующие главы его не копируют, а
# импортируют is_safe_query отсюда: две копии молча разъезжаются, и запрос,
# который блокируется в одной главе, проходит в другой.
SUSPICIOUS_PATTERNS = [
    # «систем» ловит и «системный промпт», и «системные инструкции»,
    # а вторая половина — то же самое, написанное по-английски.
    r"игнорируй.*(систем|system)",
    r"забудь.*инструкц",
    r"теперь ты можешь",
    r"новый промпт",
    r"новый системный",
...
def preload_model() -> bool:
    ...
    Возвращает False только в одном случае — модели нет в Ollama; тогда
    запускаться бессмысленно, и функция показывает установленные модели.
    Любая другая осечка (сеть моргнула, прогрев не уложился в таймаут) —
    не повод не запускаться: агент начнёт работу без прогрева, заплатив
    лишние секунды на первом ответе.
    """
    ...
    try:
        ...
        # Именно True. Раньше здесь возвращалось False, а main() на False
        # делает sys.exit(1): сообщение обещало продолжение, а программа
        # завершалась. Прогрев необязателен, а вот отсутствие модели — нет.
...
def request_model(messages: list, response_format: dict | None = None) -> str:
    ...
    передана, Ollama ограничивает генерацию на уровне токенов, и модель
    физически не может выдать невалидный JSON. Здесь параметр не
    используется: сначала мы смотрим, как ломается свободный текст.
    """
    payload = {
        "model": MODEL,
        "messages": messages,
