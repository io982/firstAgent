...
Так что глава получилась про три разные вещи, и это стоит сказать сразу:
...
* **реранкер помогает** — второй проход модели по восьмёрке кандидатов поднимает средний обратный ранг на 0.05–0.09, и это единственное место, где модель сидит внутри поиска и приносит пользу.
...
Поиск по словам нужен здесь ради одного свойства — нуля; ранжировать им не надо, замеры ниже это показали. Но чтобы понять, откуда ноль берётся, придётся разобрать, как счётчик совпадений устроен внутри.
...
Первая мысль — поставить порог на вес лучшего совпадения. Она не пережила проверки: на реалистичном наборе вопросов распределения перекрываются, и любой порог начинает отказывать настоящим вопросам (разбор — в «Что не сработало»). Осталось правило без подгоняемого числа:

> **Отказ, когда ни одно содержательное слово вопроса не встречается в проекте ни разу.**

Не «мало совпадений», а «совпадений нет вообще». Смотрит [`lexical_signal`](src/hybrid.py#L213) — доля слов вопроса, найденных в корпусе; ноль означает отказ.

Осторожность намеренно однобокая: ложное «не знаю» на настоящем вопросе — потерянный ответ, ложное «знаю» на постороннем возвращает нас к поведению Главы 5, то есть не хуже, чем было. Поэтому правило стоит в самой осторожной точке — где ложных отказов ноль.

На замере это 6 отказов из 14 посторонних вопросов и **ни одного ложного** на 20 настоящих.
...
Добавляется один — [`grep_code`](src/tools.py#L112), точное вхождение строки в файлах проекта. Таблица символов Главы 5 находит **определения** по имени; `grep_code` находит любое упоминание — в теле функции, в комментарии, в строке сообщения об ошибке. На вопрос «откуда берётся вот это сообщение» ни поиск по смыслу, ни таблица символов не отвечают.
...
Главы 5 и 6 стояли на утверждении: русский вопрос не находит английский код. Глава 5 это измерила (1 попадание из 12) и объяснила: `nomic-embed-text` на русском меряет «это вообще русская проза?», а не тему. Объяснение верное, вывод из него — нет: следует «возьмите модель, которая понимает русский», а не «постройте второй поиск». Проверка заняла один `ollama pull`:
...
Идея, с которой глава начиналась. Складывать оценки напрямую нельзя: близость лежит в отрезке от нуля до единицы, вес BM25 ничем не ограничен и на точном имени доходит до 20. Нормировка внутри выдачи меняет одну беду на другую — лучший результат любого списка становится единицей, и отличный, и лучший из плохих.
...
Список посторонних вопросов лежит в `chapter6/tests.py`, а это исходник проекта и индексируется наравне с остальными. Слово «шарлотка» попало в корпус ровно тогда, когда я его туда записал.
...
То же случилось ещё трижды: выдуманное имя класса в комментарии, потом в другом комментарии, потом в докстроке теста — каждый раз пример переставал работать через минуту после того, как я его записывал.

Лечится тем, что отказ меряется на корпусе глав 0–5 — каким репозиторий был до этой главы. Оговорка к замеру, а не к приёму: в обычном проекте «как испечь шарлотку» в исходниках не встречается. Но помнить о ней стоит всякому, кто строит поиск по корпусу, который сам же и описывает.
...
Глава нашла и починила одну настоящую дыру, и она была не в новом коде, а в старом.
...
Вывод общий: **утверждение о том, чего система не делает, — такое же утверждение, как любое другое, и проверять его надо тестом.** Тест теперь есть.

Одно новое место, которое стоит держать в голове: `grep_code` возвращает не адрес, а саму строку файла. Обход тот же, `.gitignore` работает, — но `.env.example` в индексе лежит намеренно, и его строки инструмент покажет целиком.
...
**Задание: соберите агента, который отвечает на вопросы по вашим таблицам.** Excel вместо кода — данные другие, а задачи главы те же: у поиска должен быть ноль, у ответа адрес, а у утверждения замер.
...
Задачу берите свою — ту, где вы знаете правильный ответ. Если своей нет: выгрузки из бухгалтерии, прайс-листы поставщиков, табели, отчёты по продажам за несколько лет.
