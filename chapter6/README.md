...
Полглавы было потрачено на обход препятствия, которого не было. Курс с этой главы берёт `bge-m3` по умолчанию; `AGENT_EMBED_MODEL=nomic-embed-text` возвращает старые условия, и все числа Глав 4 и 5 повторяются.
...
Индекс принадлежит модели эмбеддингов: у `bge-m3` вектор из 1024 чисел, у `nomic-embed-text` из 768, и в одной коллекции они не уживутся. Поэтому имя модели входит в имя коллекции — `chapter5_code_bge_m3`. Переключились обратно на `nomic` — заработает старый индекс, пересобирать ничего не надо.
...
| :--- | :--- |
| `AGENT_EMBED_MODEL=nomic-embed-text` | вернуть эмбеддер Глав 4–5: на гигабайт меньше памяти, заметно хуже на русских вопросах |
...
| | Глава 5 | Глава 6 |
| :--- | :--- | :--- |
| Модель эмбеддингов | `nomic-embed-text`, 274 МБ | `bge-m3`, 1.2 ГБ |
...
**`bge-m3` стоит гигабайт видеопамяти.** Вместе с `qwen2.5:3b` это всё ещё влезает в 6 ГБ, но запаса почти не остаётся, а на 4 ГБ уже нет. `AGENT_EMBED_MODEL=nomic-embed-text` возвращает прежний расход и прежнее качество поиска.
