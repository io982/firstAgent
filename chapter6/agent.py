"""
Агент Главы 6: гибридный поиск по коду (пункт 6.5).

Что меняется по сравнению с Главой 5:
  * рядом с векторным индексом появился лексический, и поиск идёт обоими
    сразу — выдачи сливаются по местам (см. src/fusion.py);
  * найденное переставляет реранкер — та же LLM, но читающая вопрос
    и фрагменты вместе (см. src/rerank.py);
  * у агента появился ответ «в проекте этого нет», и он опирается
    на число, а не на послушание модели (см. src/hybrid.py);
  * инструмент `search_code` замещён гибридным, добавлен один новый —
    `grep_code`, точное вхождение строки. Инструментов стало 16, а не 20:
    промпт Главы 5 уже занимал больше половины окна.

Маршрутизация остаётся маршрутизацией Главы 5 — она в той главе оказалась
важнее качества поиска, и трогать её здесь незачем. Меняется ровно одна
ветка: перед тем как искать, агент спрашивает лексический индекс, есть ли
в проекте вообще слова этого вопроса.
"""
import json
import os
import sys
...
# промпта; chapter6.src затем замещает search_code гибридным и добавляет
# grep_code. Поэтому `python -m chapter5.agent` остаётся Главой 5
# с векторным поиском, а гибрид видит только эта глава.
...
ДОПОЛНЕНИЕ ГЛАВЫ 6 (гибридный поиск):
