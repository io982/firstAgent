...
#
# Первая версия замера показала, что реранкер не даёт ничего, и он был
# выключен. Замер был сломан: он давал реранкеру восемь кандидатов
# и проверял попадание в первую ВОСЬМЁРКУ. Множество при перестановке
# не меняется, поэтому число попаданий не могло сдвинуться в принципе,
# а MRR мерился на списке, где ответ и так уже лежал.
#
# Агент работает иначе — берёт восемь кандидатов, переставляет, оставляет
# пять. Замер на этих двух конфигурациях (MRR, 12 вопросов):
#
#     qwen2.5:3b          0.40 → 0.45     +0.05    2.0 с на вопрос
#     qwen2.5-coder:7b    0.45 → 0.52     +0.07    4.9 с
#     qwen2.5-coder:7b    0.43 → 0.50     +0.07    4.8 с   (повтор)
#     llama3.1:8b         0.37 → 0.46     +0.09    5.4 с
#
# Помогает на всех трёх, и у 7B-coder дельта повторилась в двух прогонах.
# Движение видно поимённо: у llama3.1 `is_safe_query` уехал с четвёртого
# места на первое, `build_context` — с пятого на второе.
#
# Число попаданий при этом не меняется почти никогда: реранкер меняет
# ПОРЯДОК, а не состав. Мерить его числом попаданий бессмысленно — только
# средним обратным рангом.
#
# Выключается, если время ответа важнее порядка выдачи:
#   PowerShell:   $env:AGENT_RERANK = "0"
#   Linux/macOS:  export AGENT_RERANK=0
RERANK_ENABLED = os.environ.get("AGENT_RERANK", "1") != "0"
