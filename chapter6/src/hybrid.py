...
# По умолчанию ищем ТОЛЬКО СЛОВАМИ. Это результат замера, а не вкусовщина.
#
# На двенадцати вопросах главы (235 фрагментов, с переписыванием запроса):
#
#     только векторы    5 из 12,  MRR 0.33
#     гибрид (RRF)      7 из 12,  MRR 0.40
#     только BM25       8 из 12,  MRR 0.42
#
# Гибрид оказался МЕЖДУ своими половинами, а не над ними: векторный поиск
# на этом корпусе даёт 1 попадание из 12 без переписывания, то есть почти
# шум, и слияние его не чинит, а усредняет. Разбор — в тексте главы.
#
# Векторная половина никуда не делась и включается обратно:
#   PowerShell:   $env:AGENT_SEARCH_MODE = "hybrid"
#   Linux/macOS:  export AGENT_SEARCH_MODE=hybrid
DEFAULT_MODE = os.environ.get("AGENT_SEARCH_MODE", "bm25")
