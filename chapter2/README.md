...
| **Парсинг ответа** | Хрупкие регулярные выражения и "магия" со слешами | Constrained decoding: схема уходит в `format`, невалидный JSON невозможен |
...
Функция, а не константа — по той же причине, что и `build_system_prompt()`: Глава 3 регистрирует свои пять инструментов позже, и снимок, снятый при импорте, их бы не увидел. Глава 2 снимает свой снимок сразу (`RESPONSE_SCHEMA`), чтобы `python -m chapter2.agent` остался Главой 2 с тремя инструментами.
