...
def build_system_prompt(only: list[str] | None = None) -> str:
    ...
    `only` оставляет в промпте только названные инструменты. По умолчанию
    None — описываются все, и промпты глав 2-6 не меняются. Выборкой
    пользуется Глава 7: агент там — это не реестр, а выборка из него.
    """
