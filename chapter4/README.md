...
### Почему вопрос и документ кодируются по-разному

Тут есть тонкость, которая сначала кажется бюрократией.

Вопрос и ответ на него — тексты разной формы. Сравните: «сколько токенов в окне» и «Контекстное окно агента — 8192 токена». Общих слов почти нет, длина разная, интонация разная. Зато два вопроса между собой — «сколько токенов в окне» и «сколько стоит перебор» — похожи прекрасно: оба короткие, оба начинаются с «сколько».

Вот и вся проблема. Обычная близость отвечает на вопрос «эти два текста об одном и том же?» — а нам нужен другой: «отвечает ли этот абзац на этот вопрос?». Модель, обученная **асимметрично**, умеет второе, но ей надо сказать, какую роль играет текст. Говорят прямо в тексте, префиксом:

```python
embed_document("Контекстное окно агента — 8192 токена")  # → "search_document: Контекстное окно…"
embed_query("сколько токенов в окне")                    # → "search_query: сколько токенов…"
```

Внутри модели это разворачивает вопрос в сторону абзацев-ответов, а не в сторону других вопросов. В коде это две функции, а не одна с параметром: так нечего перепутать местами.

**А теперь замер, потому что рекомендации карточки модели тоже стоит проверять.** Шестнадцать вопросов, у каждого известно, в каком файле лежит ответ; считаем, сколько раз лучший фрагмент оказался из нужного файла:

| Как кодировали | Попаданий | Средняя близость лучшего |
| :--- | :--- | :--- |
| Правильно: документы и запросы по-разному | 10 из 16 | 0.742 |
| Без префиксов вообще | 8 из 16 | 0.740 |
| Префиксы перепутаны местами | 9 из 16 | 0.701 |
| Всё как документы | 10 из 16 | 0.757 |
| Всё как запросы | 11 из 16 | 0.744 |

Ожидание было, что правильный вариант оторвётся. Он не оторвался: разница в одно-два попадания на шестнадцати вопросах — это шум, а «всё как запросы» вообще выиграл. Единственное, что видно уверенно, — перепутанные префиксы роняют близость (0.701 против 0.742), но и там попаданий почти столько же.

Заодно посмотрите на абсолютные числа в первом столбце. Даже в лучшем варианте лучший фрагмент приходит из нужного файла в десяти случаях из шестнадцати — то есть в трети вопросов поиск ставит на первое место кусок не из того документа. Это не поломка префиксов, это уровень самого метода на таком корпусе, и к нему мы ещё вернёмся дважды: когда будем разбираться, почему не работает порог, и почему фрагментов берётся пять, а не один.

Префиксы в коде остались: так велит карточка модели, эффект описан на больших корпусах, а стоит это ноль. Но вывод главы из этой таблицы другой и важнее самих префиксов: **на маленьком корпусе половина «обязательных» рекомендаций не отличима от шума, и узнать это можно только замером на своих данных.** Шестнадцать вопросов — тоже мало; чтобы говорить о разнице всерьёз, нужны сотни.
