...
└─ system: напоминание про правила ────────── константа
   ...
   chapter4/index/knowledge.json ── векторы фрагментов, 545 КБ на 31 фрагмент
```
...
Модель называется `nomic-embed-text`, весит 274 МБ и умеет ровно одно — считать вектор из 768 чисел. Генерировать ответы она не может; это не «маленькая языковая модель», это другой инструмент. Живёт она в той же Ollama и вызывается тем же HTTP-запросом, что и `qwen2.5:3b` ([`embeddings.py`](src/embeddings.py#L94)).

Близость двух текстов — это косинус угла между их векторами ([`cosine_similarity`](src/embeddings.py#L179)). Единица — одно и то же, ноль — ничего общего. Почему угол, а не расстояние: длина вектора зависит в том числе от длины текста, а нас интересует направление — то есть смысл, — а не то, абзац перед нами или предложение.

Одна деталь, которую легко пропустить и потом полгода недоумевать. `nomic-embed-text` обучена **асимметрично**: документ и запрос кодируются по-разному, и модель ждёт, что ей об этом скажут префиксом.

```python
embed_document("Контекстное окно агента — 8192 токена")  # search_document: ...
embed_query("сколько токенов в окне")                    # search_query: ...
```

Перепутать префиксы — самая незаметная ошибка главы. Ничего не сломается, поиск продолжит работать и выдавать правдоподобные результаты. Просто хуже. Поэтому в коде это две разные функции, а не одна с параметром.
...
Фрагмент слишком большой — в контекст влезает один вместо пяти, и половина его текста не про вопрос. Слишком маленький — мысль разорвана пополам, и ни одна половина не отвечает целиком. А фрагмент без заголовка не значит вообще ничего: «поддерживается через переменную окружения» — что именно поддерживается?
...
[`MemoryVectorStore`](src/vectorstore.py#L114) — двадцать строк перебора. Нужен, чтобы увидеть: «векторный поиск» — это скалярное произведение в цикле, а не магия. Векторы хранятся нормализованными, поэтому косинус — просто произведение. Индекс лежит в обычном JSON, который можно открыть и посмотреть глазами.

[`ChromaVectorStore`](src/vectorstore.py#L228) — та же задача через настоящую базу с HNSW-индексом. Включается одной переменной окружения, остальной код не меняется ни строчкой.

Когда нужен переезд? Полный перебор — это O(n·d) на запрос: для тысячи фрагментов миллион умножений. Замер на этой машине: 31 фрагмент — 2.2 мс, тысяча — 75 мс. Против двух секунд, которые занимает эмбеддинг запроса, это ничто. Перебор перестаёт быть приемлемым не на тысячах документов, а на сотнях тысяч — и до этого момента всё, что даёт Chroma, это лишняя зависимость.
...
[`SemanticMemory`](src/semantic_memory.py#L84) строит векторный индекс поверх тех же фактов из `memory.json`. Хранилище остаётся одно, здесь только индекс над ним, и он подтягивается автоматически: факт мог быть записан минуту назад.
...
| Перебор 31 фрагмента | 2.2 мс | линейно, O(n·d) |
...
**Индекс на диске** — 545 КБ на 31 фрагмент, то есть примерно 17 КБ на фрагмент. Почти всё это — 768 чисел вектора в текстовом виде. Настоящие базы хранят их в двоичном формате и по делу; нам важнее, чтобы индекс можно было открыть и посмотреть глазами.
...
Вы: Как оформлять README главы?
 ...
 │        │ да
 │        ▼
 ├─ эмбеддинг запроса ──► перебор 31 вектора ──► 5 ближайших
 │        │
...
| `AGENT_VECTOR_STORE=chroma` | перейти с перебора на ChromaDB |
