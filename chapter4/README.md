...
Модель в замерах этой главы — `nomic-embed-text`, весит 274 МБ и умеет ровно одно — считать вектор из 768 чисел. (По умолчанию курс с некоторых пор берёт `bge-m3`: она втрое тяжелее, но заметно лучше понимает русский. Числа ниже сняты на `nomic-embed-text`, и переключатель `AGENT_EMBED_MODEL` позволяет их повторить.) Генерировать ответы она не может; это не «маленькая языковая модель», это другой инструмент. Живёт она в той же Ollama и вызывается тем же HTTP-запросом, что и `qwen2.5:3b` ([`embeddings.py`](src/embeddings.py#L106)).

Близость двух текстов — это косинус угла между их векторами ([`cosine_similarity`](src/embeddings.py#L236)). Единица — одно и то же, ноль — ничего общего. Почему угол, а не расстояние: длина вектора зависит в том числе от длины текста, а нас интересует направление — то есть смысл, — а не то, абзац перед нами или предложение.
...
Чуда от префиксов ждать не стоит: на базе знаний главы они дали 10 попаданий из 16 против 8 без них — разница в пределах шума (весь замер — в комментарии к [`DOCUMENT_PREFIX`](src/embeddings.py#L95)). Но карточка модели их предписывает, стоят они ноль, а в коде это две отдельные функции — так нечего перепутать местами.
...
Никакой магии внутри при этом нет: поиск по векторам — это скалярное произведение в цикле, и наивная реализация на двадцать строк лежит в коде рядом с базой ([`MemoryVectorStore`](src/vectorstore.py#L126)).

### Индексация, которую не страшно запустить дважды

Документы нарезаны, векторы посчитаны, база готова их принять. Осталось решить, что происходит, когда всё это запускают во второй раз — а запускают его при каждом старте агента.
...
Поиск готов: находит, фильтрует, укладывается в бюджет. Остался вопрос, кто и когда его запускает.

Проще всего отдать это модели. Поиск оформляется инструментом — таким же, как калькулятор из Главы 2, в том же реестре `@tool`: `search_docs(query)`. Дальше модель сама решает, звать его или нет. Красиво: на «привет» и «посчитай 2+2» никто никуда не ходит. Мы так и начали — и вот замер на пяти вопросах, ответы на которые есть в базе знаний:
...
У списка слов есть очевидные дыры: «сообщи, какие есть настройки» пройдёт, а «интересно узнать про пороги» — нет. Настоящий классификатор намерения потребовал бы ещё одного вызова модели на каждую реплику — это хуже, чем изредка не подложить справку.

### Найденное не оседает в истории

Найденное приходит в контекст, и напрашивается положить его в историю обычным сообщением — рядом с репликами и ответами, там же, где живёт результат любого инструмента. Так и было сделано. Сломалось на третьей реплике.
...
Второй инструмент главы, `recall_like(query)`, ищет по фактам так же, как `search_docs` по документам. Под ним [`SemanticMemory`](src/semantic_memory.py#L83) — векторный индекс поверх тех же фактов из `memory.json`. Хранилище остаётся одно, здесь только индекс над ним, и он подтягивается автоматически: факт мог быть записан минуту назад.
...
Каждый ответ агента теперь занимает больше времени, и полезно понимать, на что именно оно уходит.

| Операция | Сколько занимает | От чего зависит |
| :--- | :--- | :--- |
...
Первая строка — главная и самая обидная. **Дольше всего в поиске не перебор, а ожидание модели эмбеддингов.** Именно поэтому фильтр «не искать на утверждениях» — это не только про качество памяти, но и про две сэкономленные секунды на каждой реплике, которая ни о чём не спрашивает.
...
**Хранилище на старте отнимает 2.8 секунды** — столько занимает `import chromadb` вместе с открытием базы. Для агента, который и так прогревает модель несколько секунд, это терпимо, но цена платится при каждом запуске процесса.
...
**Окно здесь вдвое больше, чем в Главах 1–3, и это осознанный размен.** В окне на 4096 после системного промпта и резервов на разговор оставалось около 600 токенов — а один абзац документа занимает 300. Выбор был между «сократить промпт» и «расширить окно».
...
|[Назад](../chapter3/README.md) | [📚 Оглавление](../README.md) | [Далее](../chapter5/README.md)|
