...
**Положить документы прямо в промпт.** Работает, пока документы маленькие. Четыре файла с примерами из этой главы — это 31 фрагмент, примерно 7000 токенов. Окно у нас 8192, из которых больше трёх тысяч уже занял системный промпт. Не влезает даже он, а настоящая документация проекта в сто раз больше.
...
Борщ обошёл четыре настоящих вопроса из пяти. Вот он, тот самый масштаб: весь столбец умещается в 0.05 — столько же, сколько было между нужным фрагментом и случайным соседом. Порогу просто негде пройти. Нормировка на среднее по базе (z-оценка) не спасает: у борща 2.65, у вопроса про потолок поля памяти — 1.61.
