...
Близость двух текстов — это косинус угла между их векторами ([`cosine_similarity`](src/embeddings.py#L238)). Единица — одно и то же, ноль — ничего общего. Почему угол, а не расстояние: длина вектора зависит в том числе от длины текста, а нас интересует направление — то есть смысл, — а не то, абзац перед нами или предложение.
...
Чуда от префиксов ждать не стоит: на базе знаний главы они дали 10 попаданий из 16 против 8 без них — разница в пределах шума (весь замер — в комментарии к [`DOCUMENT_PREFIX`](src/embeddings.py#L97)). Но карточка модели их предписывает, стоят они ноль, а в коде это две отдельные функции — так нечего перепутать местами.
