...
Чуда от префиксов ждать не стоит: на базе знаний главы они дали 10 попаданий из 16 против 8 без них — разница в пределах шума (весь замер — в комментарии к [`DOCUMENT_PREFIX`](src/embeddings.py#L62)). Но карточка модели их предписывает, стоят они ноль, а в коде это две отдельные функции — так нечего перепутать местами.
...
Никакой магии внутри при этом нет: поиск по векторам — это скалярное произведение в цикле, и наивная реализация на двадцать строк лежит в коде рядом с базой ([`MemoryVectorStore`](src/vectorstore.py#L119)).
