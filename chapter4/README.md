...
**Положить документы прямо в промпт.** Работает, пока документы маленькие. Четыре файла с примерами из этой главы — их дальше буду называть корпусом — это 31 фрагмент, примерно 7000 токенов. Окно у нас 8192, из которых больше трёх тысяч уже занял системный промпт. Не влезает даже он, а настоящая документация проекта в сто раз больше.
...
Перебор из кода не убран: [`MemoryVectorStore`](src/vectorstore.py#L119) — это двадцать строк, по которым видно, что «векторный поиск» есть скалярное произведение в цикле. Он же удобен тестам, потому что живёт в памяти и не трогает диск. Включается переменной `AGENT_VECTOR_STORE=memory`.

Обоих закрывает [один интерфейс](src/vectorstore.py#L76): `add`, `search`, `entries`, `delete`, `clear`. Мера близости наружу тоже одна — `score` примерно от нуля до единицы, больше значит ближе. Chroma внутри считает **расстояние** (ноль — совпадение), и перевод спрятан в адаптере: иначе порог отсечения в одном хранилище значил бы «не ближе чем», а в другом — «не дальше чем».
...
**Индекс на диске** — 3 КБ на фрагмент (768 чисел по четыре байта) плюс 314 КБ на HNSW-индекс коллекции. Растёт линейно с корпусом: на десяти тысячах фрагментов это тридцать мегабайт, и это всё ещё немного.
