...
#
# Разрыв между «тем самым» фрагментом и случайным соседом — 0.02, а вся
# пятёрка укладывается в 0.024. Порогом это не поправить (см. MIN_SCORE):
# полоса слишком узкая. Пока ранжирование такое плотное, единственная
...
# случайных, порог отсекает мусор, и агент отвечает «не знаю».
...
#
# Отсюда вывод главы, и он неприятный: РЕШЕНИЕ «ОТВЕТА НЕТ» ПОИСК ПРИНЯТЬ
...
class KnowledgeBase:
    ...
    def search(
        ...
        Фильтрация — здесь, а не в хранилище: хранилище обязано ранжировать
        всё, что у него есть, без изъятий, а какие из этого фрагменты
    ...
    def build_context(self, hits: list[Hit], budget_tokens: int) -> str:
        ...
        for number, hit in enumerate(hits, 1):
            ...
            if estimate_tokens(text) > left:
                ...
                # бюджета в символы точный: left токенов ≈ 2 * left символов.
