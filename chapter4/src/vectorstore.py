...
В главе две реализации за одним интерфейсом, и это не абстракция ради
абстракции:

  * MemoryVectorStore — двадцать строк перебора. Нужен, чтобы увидеть,
    что «векторный поиск» — это скалярное произведение в цикле, а не
    магия. На корпусе главы (сотни чанков) он работает за миллисекунды
    и не требует ни одной зависимости.
  * ChromaVectorStore — та же задача через настоящую базу с HNSW-индексом.
    Нужен, когда чанков десятки тысяч и полный перебор перестаёт быть
    бесплатным.

Важно, что интерфейс один: агент не знает, какое хранилище под ним, и
переезд с перебора на базу не меняет ни строчки в остальном коде.

Единица измерения близости тоже одна — score в диапазоне примерно [0, 1],
больше значит ближе. Chroma внутри считает РАССТОЯНИЕ (0 — совпадение),
и перевод спрятан здесь: иначе порог отсечения в одном хранилище значил
бы «не ближе чем», а в другом — «не дальше чем».
"""
...
class MemoryVectorStore(VectorStore):
    ...
    Оценка сложности честная: O(n·d) на запрос, где n — число чанков,
