...
# Отдельная модель под эмбеддинги. В этой главе и в Главе 5 — nomic-embed-text:
# 274 МБ, вектор из 768 чисел, все числа обеих глав сняты на ней.
#
# ⚠️ ГЛАВА 6 ПОДНИМАЕТ ЭТУ МОДЕЛЬ ДО bge-m3 при своём импорте (см.
# chapter6/src/__init__.py) — и делает это по результату замера, а не
# из любви к большему. Двенадцать русских вопросов о коде, попадание
# нужного определения в первую пятёрку, векторный поиск без обработки
# запроса:
#
...
# Здесь она остаётся, чтобы числа этой главы и Главы 5 воспроизводились.
#
# Переменная окружения перекрывает обе главы:
#   PowerShell:   $env:AGENT_EMBED_MODEL = "bge-m3"
#   Linux/macOS:  export AGENT_EMBED_MODEL=bge-m3
...
#
# То есть на таком корпусе разница в пределах шума, и это стоит написать
# прямо, а не повторять «так велит карточка модели». Префиксы оставлены:
# карточка действительно велит, на больших корпусах эффект описан,
# а стоят они ноль.
