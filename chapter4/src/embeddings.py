...
# nomic-embed-text обучен АСИММЕТРИЧНО: документ и запрос кодируются
# по-разному, и модель ждёт, что ей об этом скажут префиксом. Без него
# качество падает — это не суеверие, а требование карточки модели.
# Документ индексируется с одним префиксом, вопрос ищется с другим.
