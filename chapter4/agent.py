...
Что изменилось по сравнению с Главой 3:
  ...
  * у найденного текста есть жёсткий потолок в токенах: он едет в то же
    окно, где уже лежат системный промпт и разговор;
...
RAG_MEMORY_RULES = """
ДОПОЛНЕНИЕ ПРО ПАМЯТЬ (Глава 4):
1. Реплика пользователя сообщает факт о НЁМ САМОМ (имя, проект, сервер, сроки, предпочтения) — это не вопрос к базе знаний. СНАЧАЛА вызови remember, и только потом отвечай. Фрагменты из документов к таким репликам отношения не имеют.
2. Ключ нового факта пиши словами и по-русски: "название сервера", "дедлайн проекта". По таким ключам находит поиск по смыслу recall_like; по ключам вида server_name он промахивается.
""".strip()
...
# Слова, по которым реплика опознаётся как просьба что-то узнать.
# Список короткий и заведомо неполный — см. looks_like_request().
REQUEST_MARKERS = (
    "?", "как ", "какой", "какая", "какое", "какие", "что ", "чем ", "чём", "где",
    "когда", "почему", "зачем", "сколько", "кто ", "куда", "можно ли", "есть ли",
    "расскажи", "покажи", "объясни", "найди", "опиши", "перечисли",
)


def looks_like_request(text: str) -> bool:
    """Похожа ли реплика на просьбу что-то узнать.

    Самый дешёвый классификатор из возможных, и появился он не от хорошей
    жизни. Автопоиск подкладывал фрагменты к КАЖДОЙ реплике — в том числе
    к «меня зовут io982», где искать нечего. И это ломало память:

        автопоиск + два вопроса по документам → remember НЕ вызван
        автопоиск выключен, те же два вопроса → remember вызван, имя в архиве
        автопоиск + «привет», «как дела»      → remember НЕ вызван

    Третья строка снимает подозрение с накопленной истории: дело не в ней,
    а в самих фрагментах. Получив пачку документов, qwen2.5:3b переключается
    в режим «отвечай по фрагментам» и перестаёт видеть инструменты — на
    реплику «меня зовут io982» она отвечала «в базе знаний нет информации
    о том, что вас так зовут». Ни правилом в промпте, ни напоминанием
    в конце контекста это не лечится: проверено, оба варианта не изменили
    ничего. Лечится тем, что поиск не зовут там, где не спрашивают.

    У списка слов есть очевидные дыры: «сообщи, какие есть настройки» пройдёт,
    а «интересно узнать про пороги» — нет. Настоящий классификатор намерения
    стоил бы ещё одного вызова модели на каждую реплику, а это дороже, чем
    изредка не подложить справку.
    """
    haystack = f" {text.lower().strip()} "
    return any(marker in haystack for marker in REQUEST_MARKERS)
...
def augment_with_context(conversation: SelectiveConversation, user_input: str) -> bool:
    ...
    И кладётся оно НЕ В ИСТОРИЮ, а в отдельное поле, которое пересобирается
    каждую реплику: почему именно так — в SelectiveConversation
    .available_history_budget(), там же замер сломанной версии.
    """
    # Прошлая справка снимается до поиска, а не после: если поиск упадёт,
    # ничего не найдёт или мы решим не искать, в контексте не должны
    # остаться фрагменты к позапрошлому вопросу.
    ...
    conversation.retrieved = f"Фрагменты из базы знаний по вопросу «{user_input}»:\n\n{context}"
    return True
...
def ensure_index() -> None:
    """Сверяет индекс с документами при каждом запуске.

    Именно сверяет, а не «строит, если пусто». Разница видна на первом же
    рабочем сценарии: документы правятся, пока агент не запущен, и агент,
    который читает вчерашний индекс, отвечает по вчерашним документам —
    молча и уверенно.

    Стоит это дёшево: неизменившиеся фрагменты не уходят в модель
    эмбеддингов, и сверка занимает доли секунды (см. IndexReport — в нём
    видно, сколько было пересчитано на самом деле).
    """
...
if __name__ == "__main__":
    ...
    while True:
        user_input = input("\nВы: ")
        # Команда узнаётся по очищенной строке: лишний пробел в конце —
        # обычное дело при копировании, а «индекс » молча уехало бы в модель.
