...
Слот, который у Letta занимает core memory — блок, всегда видимый модели, —
здесь занят другим содержимым. Не профилем, который правит агент, а пересказом,
который пишет оркестратор. По уровням памяти это перенесённая через перезапуск
working memory:
...
Работает лениво и по просьбе человека:

    stash()    — при выходе сложить хвост разговора на диск как есть.
                 Мгновенно, без модели: выход не должен стоить ожидания.
    condense() — пересказать сложенное. Стоит запроса к LLM, поэтому
                 делается, когда человек набрал «продолжить», а не при
                 каждом запуске агента.

Платим за это дрейфом: пересказ пересказывает пересказ, и с каждым кругом
частностей меньше, а выдумок больше. Поэтому здесь есть счётчик `depth`,
журнал с длиной каждого сохранения и страж письменности (foreign_letters):
деградацию должно быть видно, а не подозреваться.
"""
...
def _is_expected_letter(char: str) -> bool:
    """Буква из ожидаемой письменности: латиница или кириллица."""
    code = ord(char)
    return (
        "a" <= char.lower() <= "z"
        or 0x00C0 <= code <= 0x024F      # латиница с диакритикой
        or 0x0400 <= code <= 0x052F      # кириллица
    )


def foreign_letters(text: str, source: str = "") -> set[str]:
    """Буквы чужой письменности, которых не было в исходном тексте.

    Зачем это нужно, видно только на живом файле. Вот настоящий пересказ
    после двух кругов сжатия на qwen2.5:3b:

        io ранее обсуждал информацию о себе, включая своё имя, планы на锻炼,
        режим питания...

    Модель обучена в основном на китайском, и, пересказывая собственный
    пересказ, она подставила китайское слово вместо русского. Промптом это
    не лечится: на втором круге текст всё дальше от того, что говорил человек,
    и модель всё свободнее обращается с формой.

    Сравниваем с исходником, а не просто ищем иероглифы: если пользователь
    сам обсуждал китайский текст, он имеет право остаться в пересказе.
    """
    known = {char for char in source if char.isalpha() and not _is_expected_letter(char)}
    return {
        char for char in text
        if char.isalpha() and not _is_expected_letter(char) and char not in known
    }
...
class PreviousSession:
    ...
    def condense(self, summarizer_fn=None, facts: dict | None = None) -> bool:
        ...
        saved = self.save(summary, chained=chained, source=self.summary + self.pending)
        if saved.startswith("❌"):
            return False
    ...
    def save(self, summary: str, chained: bool = False, source: str = "") -> str:
        ...
        Args:
            ...
            source: текст, из которого пересказ сделан. Нужен, чтобы отличить
                «модель съехала в чужой алфавит» от «в разговоре и был китайский».
        """
        ...
        # Дрейф пересказа проявляется не только смыслом, но и письменностью.
        strange = foreign_letters(summary, source)
        if strange:
            self._log(f"ОТКАЗ  чужой алфавит в пересказе: {''.join(sorted(strange))[:20]}")
            return (
                f"❌ В пересказе появились символы чужого алфавита "
                f"({''.join(sorted(strange))[:10]}), которых не было в разговоре. "
                f"Оставляю прежний пересказ."
            )
...
def enrich_with_facts(summary: str, facts: dict, model_fn=None) -> str:
    ...
    # Четыре причины отказаться от результата, и все четыре наблюдаемы:
    ...
    if foreign_letters(enriched, summary + " ".join(str(v) for v in facts.values())):
        return summary                      # модель съехала в чужую письменность
