...
* 📄 **[chapter3/tests.py](tests.py)** — 88 быстрых тестов + 10 интеграционных с живой моделью.
...
| :--- | :--- | :--- |
| Системный промпт | ~1980 токенов | Постоянные расходы, платятся **каждый** запрос |
...
Подставьте числа: 4096 − 1977 − 27 − 600 = **1492 токена на всю историю разговора**. Это тот самый ответ на вопрос «сколько мы вообще можем себе позволить помнить», и он получается арифметикой, а не на глаз. Поменяете промпт — бюджет пересчитается сам, поменяете размер окна — тоже.

Агент печатает расклад при каждом запросе, так что цифры видно вживую:

```
📊 Отправляем 7 сообщений, ~1966 токенов из 4096
```
...
| ---: | ---: |
| 4096 (по умолчанию) | ~1500 токенов |
| 8192 | ~5600 токенов |
| 32768 | ~30000 токенов |
...
**2. Видеопамять есть не у всех.** Курс рассчитан на 4 GB VRAM. Модель 3B при 32768 займёт 3.4 GB — почти всё, и на эмбеддинги из [Главы 4](../chapter4/README.md) уже не останется. При 4096 запас есть.

**3. Большое окно не решает задачу, а отодвигает её.** Внимание модели деградирует задолго до формального предела: инструкция из середины 30-тысячного контекста для 3B-модели почти невидима — это тот самый третий сценарий из раздела про overflow. Плюс каждый заполненный токен оплачивается временем prefill при **каждом** запросе. Агент, который держит в контексте 30 тысяч токенов, работает медленно и невнимательно, даже если формально «всё влезает».
...
> ⚠️ **Честно о цене этой главы.** Промпт Главы 2 занимал 803 символа. Мы добавили правила контекста, правила безопасности, описания пяти инструментов памяти и few-shot примеры — и получили 3955. Глава про экономию контекста раздула постоянные расходы **почти в пять раз**, съев 48% окна.
>
> Это не оплошность, а реальный размен: маленькая модель без подробных инструкций и примеров не попадает в формат вызова. Но помнить о размене надо. Способ съесть пирог целиком — не держать все инструменты в промпте постоянно, а подгружать релевантные под задачу. Это уже RAG, и это [Глава 4](../chapter4/README.md).
...
if not tool_calls:
    ...
    conversation.add("assistant", answer)   # ← без этой строки беседа рвётся
    return answer
```
...
Ещё один подход: не резать по позиции, а выбирать по релевантности — из всей истории поднять те сообщения, что относятся к текущему вопросу. Это требует умения измерять смысловую близость, то есть эмбеддингов. Отложим до [Главы 4](../chapter4/README.md).
...
def compact(self) -> bool:
    ...
    if not summary:
        self.history = recent              # сжать не вышло — выбрасываем старое:
        return False                       # потерять часть истории лучше,
                                           # чем переполнить контекст
...
Соблазн был завести для памяти отдельный словарь инструментов и отдельный диспетчер: инструменты «свои», логика «особая». Это было бы ошибкой — Глава 2 ровно за тем и вводила реестр, чтобы такого больше не делать. Поэтому инструменты памяти регистрируются тем же декоратором:
...
Обходной путь у агента есть: вызвать `list_memories`, посмотреть на все ключи и выбрать нужный. Работает, пока фактов десятки. На сотнях весь список перестанет влезать в контекст — и мы вернёмся к проблеме, с которой начали главу.

Настоящее решение — искать по смыслу, а не по точному совпадению. Для этого нужны эмбеддинги и векторная база, и это ровно тема [Главы 4](../chapter4/README.md).
...
>
> Настоящие границы строятся не в промпте, а в архитектуре: права инструментов, песочница, подтверждение человеком перед опасными действиями. Это отдельная глава — см. [ROADMAP.md](../ROADMAP.md).
...
Итог главы по безопасности честный: промпт-защиты **снижают** вероятность, а не устраняют её. Пока последнее слово за моделью, любая такая защита — вопрос статистики. Настоящие границы строятся там, где модель ничего не решает: права инструментов, песочница, подтверждение человеком. Это [Глава 12](../ROADMAP.md).
...
graph TD
    ...
    D2 --> E["request_model"]
    E --> F["extract_tool_calls"]

    F -- "JSON не найден" --> Y["Ответ в историю и пользователю"]
    F -- "Найден вызов" --> G["execute_tool — один на все восемь инструментов"]
...
Быстрых тестов 88, они не требуют ни Ollama, ни сети и проходят за доли секунды. Интеграционных 10, по умолчанию они пропускаются (`pytest.ini`), требуют запущенной Ollama и уважают переменную `AGENT_MODEL`.
...
| `TestConversation` | История переживает реплику; **резюме считается один раз**; старое резюме не теряется при повторном сжатии; `reset` чистит диалог, но не память |
...
| `TestLongTermMemory` | Полный цикл фактов; персистентность через перезапуск; `tmp_path` вместо боевого файла |
...
graph TD
    ...
    subgraph B["chapter2"]
        B1["src/tools.py<br/>@tool, TOOL_REGISTRY, execute_tool, describe_tools"]
        B2["agent.py<br/>build_system_prompt, extract_tool_calls, is_safe_query"]
        B1 --> B2
    end
...
| Поиск в памяти — только по точному ключу | Без эмбеддингов иначе никак | [Глава 4](../chapter4/README.md) |
