...
У этого есть приятный побочный эффект: пересказ стоит запроса к модели, и платят за него только те запуски, где он понадобился. Кому прошлое нужно всегда — `AGENT_RESUME=1`, и блок поднимется сразу.
...
**Пока он поднят, за него платит история.** 303 токена — это примерно четверть того, что остаётся на разговор в окне 4096. Ровно поэтому блок и не поднимается сам.
...
| Хвост разговора лежит на диске сырым, пока его не попросят пересказать | плата за мгновенный выход |
