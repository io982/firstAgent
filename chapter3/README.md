...
├─ user:   [PREV_SESSION] пересказ прошлого разговора ── 300 токенов резерва
├─ user:   [SUMMARY] резюме сжатой части этой сессии ── появляется не сразу
├─ …       свежая история: реплики, вызовы, результаты ── 741 токен, режется
...
Взвешивает сообщения [`estimate_tokens`](src/context.py#L45), и это честная грубая прикидка: длина текста пополам. Для русского это примерно верно, ошибка легко достигает трети, и нам достаточно — вопрос стоит «мы близко к краю или нет», а не «сколько ровно».

Системного промпта в истории нет вообще. Он подставляется отдельно в [`build_messages`](src/context.py#L346), и поэтому его физически невозможно обрезать. Это надёжнее, чем помнить про «не трогай нулевой элемент» в каждой функции.
...
Чтобы такое вообще можно было различить, результаты инструментов помечаются префиксом при записи в историю — [`add_observation`](src/context.py#L329). И роль у них `user`, а не `assistant`: это сообщение внешнего мира, а не слова модели. Иначе модель начинает считать выдуманные ею данные собственным знанием.
...
То что здесь реализовано это **заглушка**. Правильнее искать по истории: сложить все прошлые реплики в индекс и доставать оттуда релевантные под текущий вопрос. Это RAG, для него нужны эмбеддинги и векторная база, и это  тема следующей главы.

* при выходе на диск падает **сырой хвост** разговора — [`stash`](src/previous_session.py#L192). Никакой генерации, просто запись файла, мгновенно;
* при следующем запуске этот хвост пересказывается — [`condense`](src/previous_session.py#L222). 
...
Есть и второй проход. Пересказ обезличивает: суммаризатор пишет «пользователь чинил индексацию», даже когда имя лежит в файле фактов. Поэтому готовый текст показывается модели вместе с фактами из архива — [`enrich_with_facts`](src/previous_session.py#L367), — и она подставляет туда имя. Отдельным промптом, а не добавкой к промпту суммаризатора: тот выстрадан и трогать его ради удобства нельзя.

Опыт показал, что на вход этому проходу нельзя подавать весь архив. Модель втягивает в пересказ всё, что видит: разговор про индексацию каталога превращается в «индексацию на сервере prod-01» просто потому, что такой сервер упомянут где-то в файле. Правдоподобно — и потому опасно. Поэтому [`relevant_facts`](src/previous_session.py#L343) отбирает только личность и то, что в пересказе уже упомянуто: подстановка уточняет, а не досочиняет.
...
### Чего заглушка не умеет

Стоит понимать её границы, иначе она будет вас подводить незаметно.
...
Что сколько стоит в запросах к модели:
...
```
окно                                  4096
− системный промпт                    2425   ← инструменты, правила, примеры
− напоминание в конце                   27
− резерв под пересказ прошлой сессии    303   ← по верхней границе, не по факту
− место под ответ модели                600
────────────────────────────────────────────
= бюджет истории                       741 токен
```
...
4. Поговорите о чём-нибудь конкретном, наберите `выход` (он мгновенный) и запуститесь снова: при старте увидите `💭 Пересказываю прошлую сессию`, а на вопрос «о чём мы говорили в прошлый раз?» получите ответ.
...
Команды REPL: `забудь` — очистить историю разговора, `сессия` — показать пересказ и журнал, `забудь сессию` — стереть память о прошлых разговорах, `выход` — завершить.
...
| Пересказ дрейфует и выдумывает связи | каждый круг — новая генерация поверх предыдущей |
| Хвост разговора лежит на диске сырым до следующего запуска | плата за мгновенный выход |
...
* ✅ помнить, о чём был прошлый разговор, не заставляя вас ждать на выходе;
