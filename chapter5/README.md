...
**Позднее выяснилось, что дело было в модели.** Замер следующей главы взял вместо `nomic-embed-text` многоязычную `bge-m3` — и тот же векторный поиск на тех же двенадцати вопросах дал 10 попаданий из 12 вместо одного. Оговорка выше оказалась не формальностью: проверена была одна модель, и вывод «векторы не находят русский вопрос» относился к ней, а не к векторному поиску. Числа этого раздела сняты на `nomic-embed-text` и остаются верными для неё.
...
Все числа выше получены на одной машине, с одной моделью эмбеддингов (`nomic-embed-text`), на одном корпусе, с вопросами на одном языке. Последнее из этих условий оказалось решающим: на многоязычной `bge-m3` те же двенадцать вопросов дают 10 попаданий из 12 вместо одного — замер в следующей главе. Курс с тех пор берёт `bge-m3` по умолчанию, а `AGENT_EMBED_MODEL=nomic-embed-text` возвращает условия этой главы. Замеры с участием LLM повторены дважды и разъехались на единицу (6 и 7 из 12) — остальные прогонялись по одному разу. Разница между 1 и 6–7 из 12 велика настолько, что вряд ли объясняется шумом; разница между 1 и 1 не говорит ничего ни о карточках вообще, ни о других моделях эмбеддингов.
