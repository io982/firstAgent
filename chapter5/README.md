...
* **документация не индексируется вместе с кодом** ([`INDEX_DOCS`](src/codebase.py#L73)). Тексты глав остаются корпусом Главы 4, а выбирает между корпусами маршрутизация. Почему не вместе — в разделе «Что не сработало».

### Переписываем вопрос на язык кода

Русский вопрос и английский код далеки друг от друга, и подтянуть к вопросу документ не выходит — почему, разобрано в разделе «Что не сработало». Остаётся подтянуть вопрос к документу: перед поиском просим **ту же самую модель** превратить вопрос в набор вероятных имён из кода ([`rewrite_query`](src/rewrite.py#L100)). Примеры формата:
...
Сравниваем два режима: подложить справку в контекст **до** первого вызова модели или оставить модели выбор инструмента. Выигрывает первый, и с большим отрывом; разбор этого замера — в разделе «Что не сработало».
