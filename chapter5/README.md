...
Половина работы над главой ушла на приёмы, которые ничего не дали, и на варианты, которые пришлось отбросить. Они собраны здесь, отдельно от того, что работает, — но не выброшены: за каждым стоит замер, а часть из них осталась в коде переключателями, чтобы можно было проверить на своём проекте.

### Карточка фрагмента: приписываем к коду его описание

Главная несбывшаяся надежда главы.
...
### Индекс кода вместе с документацией
...
У части реплик есть свой инструмент: посчитать, узнать погоду, записать факт. Подкладывать им в контекст найденное казалось безобидным — лишняя справка не мешает. Мешает: с подсказками в контексте модель звала инструмент втрое реже, чем без них. Провалилась даже арифметика. «Сколько будет 4568+5?» подходит под маркер вопроса «сколько», автопоиск Главы 4 подкладывал документы — и модель считала сама, без калькулятора. Ответ при этом был верным, что хуже всего: тихо считать в уме 3B умеет ровно до тех пор, пока числа маленькие.

### Выбор инструмента моделью вместо маршрутизации

Замер сравнивал два режима: подложить справку в контекст до первого вызова модели или оставить выбор инструмента модели. Во втором инструменты вызывались почти всегда — и почти вдвое реже давали верный ответ. Это та же беда, что измерена в Главе 4, только с другой стороны: получив пачку фрагментов, 3B переключается в режим «отвечай по фрагментам» и перестаёт видеть инструменты; а оставшись без фрагментов, она зовёт инструмент, но выбирает не тот.
...
Инструмент `dependencies` был у модели всё это время. Она его не позвала.

### Близость в шапке найденного фрагмента

В Главе 4 шапка найденного фрагмента несла близость, и здесь она сначала осталась. Живой прогон её и убрал: получив «(близость 0.70)», модель переписала это число в ответ пользователю. Нам оно нужно для отладки, а человеку в ответе «калькулятор реализован в `chapter1/agent.py` (близость 0.70)» оно не говорит ничего.

### Настоящий парсер для JavaScript

`tree-sitter` разобрал бы JS и TS по-настоящему — с методами классов, JSX и регулярными выражениями, на которых спотыкается скобочный сканер. В главу он не взят: это внешняя зависимость с бинарными колёсами, а курс обещал стандартную библиотеку. Сканер вместо парсера — размен, у которого видны обе стороны: границы его ошибок перечислены выше, а индексация от них не падает.
