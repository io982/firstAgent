...
Фрагменты кода живут в отдельной коллекции `chapter5_code`, в той же базе, что и документы Главы 4 ([`get_code_store`](src/codebase.py#L100)). Складывать их в одну коллекцию нельзя: короткая функция на пять строк конкурировала бы там с абзацем документации, а потолок выдачи у них разный.

Идентификатор фрагмента — хэш его текста и адреса, как в Главе 4. Из этого одного следует всё поведение [`index`](src/codebase.py#L156): функция не менялась — id тот же, и в модель эмбеддингов она повторно не уходит; функцию поправили — id новый, а старая запись попадает в список устаревших и удаляется. Без удаления агент через неделю отвечал бы кодом, которого в файле уже нет, — для кода это не теоретическая неприятность, а норма жизни.
...
Найденное едет в то же окно, где уже лежат промпт и разговор, поэтому [`build_context`](src/codebase.py#L244) собирает фрагменты под явный потолок в токенах. Устроено как в Главе 4, с двумя отличиями.
