...
graph TD
    subgraph IDX["Один раз при индексации"]
        ...
        Cut --> Card["Карточка фрагмента: к коду<br/>приписывается его описание —<br/>имя, сигнатура, докстрока"]
...
Размер фрагмента задают константы, и каждая посчитана от бюджета выдачи, а не выбрана на глаз:
...
**Для JavaScript и TypeScript разбора нет** — настоящий парсер этого языка живёт внутри движка JavaScript, а внешний в главу не взят. Вместо разбора — [скобочный сканер](src/codechunks.py#L556): заголовок определения ищется в начале строки на нулевой глубине вложенности, конец — там, где фигурные скобки снова сошлись в ноль. Состояние в [`_scan_line`](src/codechunks.py#L456) пропускает скобку внутри строки, внутри комментария и внутри многострочного шаблона в обратных кавычках — там, где наивный подсчёт ломается.
...
Последствия ошибки при этом ограничены: фрагмент получается больше нужного или с чужим хвостом, но индексация не падает и файл не теряется. Проверять есть на чём: в [`samples/`](samples) лежат `todo.js` и `store.ts`, где эти ловушки собраны нарочно.
...
Обход идёт через `os.walk`, а не через `rglob`, ровно по одной причине: `os.walk` позволяет **отрезать** папку целиком, а `rglob` сначала зайдёт в `node_modules` и перечислит все сорок тысяч файлов, чтобы мы их потом отфильтровали ([`iter_sources`](src/languages.py#L209)). На фронтенд-проекте разница между ними — секунды против минут.
...
* **тесты остаются в индексе**, но на вопрос «где реализовано» отвечает реализация: в поиске и в перечислениях они уезжают в хвост;
* **документация не индексируется вместе с кодом** ([`INDEX_DOCS`](src/codebase.py#L73)). Тексты глав остаются корпусом Главы 4, а выбирает между корпусами маршрутизация.

### Индекс кода: свой корпус и своя сверка

Фрагменты кода живут в отдельной коллекции `chapter5_code`, в той же базе, что и документы Главы 4 ([`get_code_store`](src/codebase.py#L110)). Складывать их в одну коллекцию нельзя: короткая функция на пять строк конкурировала бы там с абзацем документации, а потолок выдачи у них разный.

Идентификатор фрагмента — хэш его текста и адреса, как в Главе 4. Из этого одного следует всё поведение [`index`](src/codebase.py#L168): функция не менялась — id тот же, и в модель эмбеддингов она повторно не уходит; функцию поправили — id новый, а старая запись попадает в список устаревших и удаляется. Без удаления агент через неделю отвечал бы кодом, которого в файле уже нет, — для кода это не теоретическая неприятность, а норма жизни.

Сверка идёт при **каждом** запуске, а не только когда индекс пуст. Причина та же, что в Главе 4, только острее: документы правят раз в неделю, код — каждый день, и индекс, собранный вчера, отвечает вчерашним кодом.

### Переписываем вопрос на язык кода

Русский вопрос и английский код далеки друг от друга, и подтянуть документ к вопросу не выходит. Остаётся подтянуть вопрос к документу: перед поиском просим **ту же самую модель** превратить вопрос в набор вероятных имён из кода ([`rewrite_query`](src/rewrite.py#L100)). Примеры формата:
...
На том же наборе вопросов переписывание поднимает попадание в разы — это самая большая разница, какую в главе дал один приём, и получена она одним запросом к модели, а не новой моделью эмбеддингов. Половина вопросов при этом не находится по-прежнему.
...
**Часть вопросов о коде не нуждается в эмбеддингах вовсе.**
...
Из карты собираются справки — готовые ответы, которые агент кладёт в контекст вместо найденных фрагментов. Все они точные:
...
Что перечисление обязано различать:
...
Сравниваем два режима: подложить справку в контекст **до** первого вызова модели или оставить модели выбор инструмента. Выигрывает первый, и с большим отрывом.

Отсюда [`route`](agent.py#L771) — проверки в жёстком порядке, и первым стоит **разбор**, а не поиск:
...
Как агент понимает, что вопрос про код ([`looks_like_code_question`](agent.py#L255)) — признаки идут от самого лёгкого к самому тяжёлому:
...
Найденное едет в то же окно, где уже лежат промпт и разговор, поэтому [`build_context`](src/codebase.py#L256) собирает фрагменты под явный потолок в токенах. Устроено как в Главе 4, с двумя отличиями.
...
**Обрезка идёт по строкам** ([`fit_lines`](src/codebase.py#L437)) и проговаривается вслух — оборванная посреди строки скобка сбивает модель сильнее, чем отсутствие последних трёх строк функции. Если не влезает даже начало, фрагмент выбрасывается целиком: полтора заголовка функции не отвечают ни на что.

Одно правило ранжирования, которого в Главе 4 не было. **Тест — не реализация** ([`demote_tests`](src/codebase.py#L366)). На вопрос «где реализован калькулятор» в выдачу попали `chapter3/tests.py:1721` и `chapter2/tests.py:86` — места, где `calculator` вызывается, — и агент назвал их наравне с настоящей реализацией. Понижение мягкое: тесты не выбрасываются, а уезжают в хвост, и если спросили именно про тесты, порядок не трогается вовсе.

---

## 🧪 Что не сработало

Половина работы над главой ушла на приёмы, которые ничего не дали. Они собраны здесь, отдельно от того, что работает: за каждым стоит замер, а часть осталась в коде переключателями.

### Карточка фрагмента: описание, приписанное к коду

Главная несбывшаяся надежда главы. Вопрос «где реализован калькулятор» и текст `def calculator(expression: str)` написаны на разных языках, общих слов у них нет. Но материал для связки лежит в самом коде — имя, сигнатура, докстрока, путь, — и [`build_card`](src/cards.py#L118) собирает из него связный текст, который приклеивается к коду **перед** эмбеддингом:
...
Идентификаторы при этом [разбираются на слова](src/cards.py#L77): `chunkTextWithLines` для модели эмбеддингов одно незнакомое слово, `chunk text with lines` — четыре знакомых. В базу всё равно уезжает код — [эмбеддится один текст, а возвращается другой](src/cards.py#L157), и пользователь видит файл, а не наши приписки к нему.

На корпусе курса это не дало ничего: русский вопрос без слов из кода находит нужное определение одинаково редко во всех трёх режимах — на голом коде, на карточке вместе с кодом и на одной карточке. Причину видно на прямом измерении близости: посторонняя русская фраза оказывается к вопросу ближе, чем карточка с правильной докстрокой внутри. **`nomic-embed-text` на русском меряет «это вообще русская проза?», а не тему**, и приписать к английскому коду немного русского — значит сдвинуть вектор в сторону всей русской прозы разом, а не в сторону этого вопроса. Обобщать при этом не стоит: проверены одна модель эмбеддингов, один корпус и один язык вопросов, и все три режима остались переключателем `AGENT_CODE_EMBED`.

**Позднее выяснилось, что дело было в модели.** Замер следующей главы взял вместо `nomic-embed-text` многоязычную `bge-m3` — и тот же векторный поиск на тех же двенадцати вопросах дал 10 попаданий из 12 вместо одного. Оговорка выше оказалась не формальностью: проверена была одна модель, и вывод «векторы не находят русский вопрос» относился к ней, а не к векторному поиску.

Эта глава остаётся на `nomic-embed-text` — чтобы её числа воспроизводились так, как здесь написано. Модель поднимает следующая глава, у себя.

### Документация в одном индексе с кодом

Первый индекс репозитория собирался вместе со всеми `.md`, и вопрос «где вычисляется арифметическое выражение» дал на нём первую шестёрку из одних README — ни одного фрагмента кода. Проза о коде выигрывает у самого кода на его же вопросах, поэтому `AGENT_CODE_DOCS=0`, а тексты глав остаются корпусом Главы 4.

### Подсказки в контексте на задачах для инструментов

У части реплик есть свой инструмент: посчитать, узнать погоду, записать факт. Подкладывать им найденное казалось безобидным — на деле с подсказками модель звала инструмент втрое реже, чем без них. Провалилась даже арифметика: «Сколько будет 4568+5?» подходит под маркер вопроса «сколько», в контекст приезжали документы, и модель считала сама. Ответ был верным, что хуже всего: в уме 3B считает ровно до тех пор, пока числа маленькие.

### Выбор инструмента моделью вместо маршрутизации

Оставленная сама себе, модель звала инструменты почти всегда — и почти вдвое реже давала верный ответ. Получив пачку фрагментов, 3B переключается в режим «отвечай по фрагментам» и перестаёт видеть инструменты; оставшись без фрагментов, зовёт инструмент, но не тот. На вопросе «кто импортирует `chapter3.src.context`» она выдумала два импорта, которых нет, — при живом `dependencies` в реестре.

### Близость в шапке найденного фрагмента

В Главе 4 шапка несла близость, здесь она сначала осталась. Живой прогон её убрал: получив «(близость 0.70)», модель переписала число в ответ пользователю. Нам оно нужно для отладки, человеку в ответе не говорит ничего.

### Настоящий парсер для JavaScript

`tree-sitter` разобрал бы JS и TS по-настоящему, вместе с методами классов и JSX. В главу он не взят: внешняя зависимость с бинарными колёсами против обещания курса «на стандартной библиотеке». Скобочный сканер ошибается предсказуемо, а индексация от его ошибок не падает.

---

## 🛡️ Безопасность

Проверка на инъекцию из Главы 1 по-прежнему стоит первой, до всякого поиска, а вывод инструментов по-прежнему оборачивается служебными метками Главы 3. Но глава добавляет к поверхности вот что.
...
А вот что поверхность сузило, а не расширило. Просьба «покажи содержимое файла» обрабатывается [нашим кодом](agent.py#L646), а не решением модели: файл ищется в карте проекта и только потом читается через `read_file` Главы 2 — то есть прочитан будет файл из индекса, а не произвольный путь, который придумала модель. И финальный ответ [очищается](agent.py#L147) от служебных меток `[TOOL_OUTPUT_START]`: в живом прогоне ответ начинался с них, и выглядело это поломкой, даже когда по сути был верным.
...
Режимы `AGENT_CODE_EMBED` на нарезке по определениям ([`test_embedding_modes`](tests.py#L2277)):
...
Тот же набор вопросов, тот же индекс ([`test_query_rewriting`](tests.py#L2254)):
...
**Первая индексация — единственная долгая операция главы**, и уходит на неё ожидание модели эмбеддингов, а не перебор. Повторная сверка почти ничего не считает: неизменившиеся фрагменты в модель не уходят, и на всю проверку остаётся чтение файлов и сравнение идентификаторов.
...
Все числа выше получены на одной машине, с одной моделью эмбеддингов (`nomic-embed-text`), на одном корпусе, с вопросами на одном языке. Условие про модель оказалось решающим: на многоязычной `bge-m3` те же двенадцать вопросов дают 10 попаданий из 12 вместо одного — замер в следующей главе. Здесь всё считается на `nomic-embed-text`, и `AGENT_EMBED_MODEL=bge-m3` позволяет увидеть разницу своими глазами. Замеры с участием LLM повторены дважды и разъехались на единицу (6 и 7 из 12) — остальные прогонялись по одному разу. Разница между 1 и 6–7 из 12 велика настолько, что вряд ли объясняется шумом; разница между 1 и 1 не говорит ничего ни о карточках вообще, ни о других моделях эмбеддингов.
